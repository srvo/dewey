...
class DropSmallTablesScript(BaseScript):
    ...
    def __init__(self):
        ...
        super().__init__(
            name="drop_small_tables",
            description="Drop tables with fewer than N rows"
        )
    ...
    def setup_argparse(self):
        ...
        parser.add_argument(
            "--output",
            type=Path,
            help="Path to save list of dropped tables"
        )
        return parser
    ...
    def get_table_counts(self) -> List[Tuple[str, int]]:
        ...
        """
        results = []
        tables = self.db_engine.execute("SELECT table_name FROM duckdb_tables()").fetchall()
        
        for (table,) in tables:
            try:
                count = self.db_engine.execute(f'SELECT COUNT(*) FROM "{table}"').fetchone()[0]
    ...
    def drop_tables(self, tables: List[str], dry_run: bool = True) -> None:
        ...
        for table in tables:
            try:
                ...
                self.logger.info(f"{'Would drop' if dry_run else 'Dropped'} table: {table}")
            except Exception as e:
                self.logger.error(f"Error dropping table {table}: {e}")
    
    def save_results(self, dropped_tables: List[Tuple[str, int]], output_path: Path) -> None:
    ...
    def run(self) -> None:
        ...
        to_drop = [(table, count) for table, count in table_counts if count < self.args.min_rows]
