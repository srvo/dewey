...
def get_hook_errors() -> Dict[str, str]:
    ...
    if not sys.stdin.isatty():
        ...
        file_error_pattern = re.compile(r'([^\s]+\.[^\s]+).*?\n(.*?)(?=\n[^\s]+\.[^\s]+|\Z)', re.DOTALL)
...
def parse_pre_commit_output() -> Tuple[Dict[str, List[str]], List[str], List[str]]:
    ...
    if not sys.stdin.isatty():
        try:
            ...
            syntax_error_pattern = re.compile(r'(?:error: |Syntax error in )([^:]+):[^:]+: (Expected an indented block|[^\n]+)')
...
def stage_files(files: List[str]) -> bool:
    ...
    try:
        ...
        if len(files) > chunk_size:
            print(colorize(f"Staging {len(files)} files in chunks of {chunk_size}...", "1;33"))
            ...
            for i in range(0, len(files), chunk_size):
                ...
                try:
                    ...
                    print(colorize(f"Staged files {i+1}-{min(i+chunk_size, len(files))} of {len(files)}", "1;32"))
                except subprocess.CalledProcessError as e:
                    print(colorize(f"Error staging chunk {i//chunk_size + 1}: {e}", "1;31"))
                    success = False
        else:
...
def run_aider_with_fallback(file_path: str, error_message: str) -> None:
    ...
    try:
        ...
        if "Expected an indented block" in error_message or "Syntax error" in error_message:
            ...
            prompt = f"Fix the following pre-commit hook error in this file: {error_message}"
        ...
        if includes_conventions:
            subprocess.run(["aider", "--message", prompt, conventions_path, file_path], check=True)
        else:
...
def select_file_menu(files: List[str], action: str) -> Optional[str]:
    ...
    try:
        ...
        print(colorize("Cannot read input. Run this script directly for interactive mode.", "1;31"))
        return None
...
def display_hook_summary(hook_results: Dict[str, List[str]]) -> None:
    ...
    if not hook_results:
        ...
        print(colorize("  pre-commit run | python scripts/post_hooks_guidance.py", "1;36"))
        return
    
    for hook, files in hook_results.items():
        if files:
            file_count = len(files)
            print(colorize(f"\n{hook}: ", "1;33") + colorize(f"Modified {file_count} files", "1;32"))
...
def display_syntax_errors(syntax_errors: List[str]) -> None:
    ...
    print(colorize("\n=== SYNTAX ERRORS ===", "1;31"))
    print(colorize("The following files have syntax errors that need to be fixed:", "1;37"))
...
def show_interactive_menu(modified_files: List[str], errors: Dict[str, str], hook_results: Dict[str, List[str]], failed_hooks: List[str], syntax_errors: List[str]) -> None:
        ...
        try:
            ...
            if choice == "1":
                ...
                if file:
                    ...
                    if not error_msg:
                        error_msg = errors.get(file, "Fix any code quality issues in this file")
                ...
                if failed_hooks:
                    print(colorize("\nThe following hooks failed without modifying files:", "1;31"))
                ...
                print(colorize("\nInvalid choice. Please enter a number between 1 and 7.", "1;31"))
        except EOFError:
            # Handle the case where we can't read from stdin
            print(colorize("\nCannot read input. Run this script directly for interactive mode.", "1;31"))
            break
...
def print_guidance():
    ...
    if syntax_errors:
        print(colorize("\nCRITICAL: ", "1;31") + colorize(f"Found {len(syntax_errors)} syntax errors that must be fixed before committing!", "1;37"))
        print(colorize("These are usually due to missing indentation blocks in Python files.", "1;37"))
    ...
    if hook_results:
        ...
        if hooks_that_fixed:
            print(colorize("\nIMPORTANT: ", "1;31") + colorize("Some hooks reported as 'Failed' actually FIXED issues in your files.", "1;37"))
            print(colorize("This is normal - it means pre-commit automatically corrected problems for you.", "1;37"))
            print(colorize("You need to STAGE these changes before you can commit.", "1;37"))
    ...
    if modified_files:
        ...
        if not sys.stdin.isatty() or not sys.stdout.isatty():
            ...
            if len(modified_files) > 10:
                ...
                print("\n" + colorize("Run these commands to stage changes and commit:", "1;32"))
                print(f"  git add {files_str}")
            
            print("  git commit -m \"Your message\"")
            print("\n" + colorize("Or run this script directly for interactive options:", "1;36"))
            print(f"  python {os.path.basename(__file__)}")
        
        # Only show interactive menu if in interactive mode
        show_interactive_menu(modified_files, errors, hook_results, failed_hooks, syntax_errors)
    else:
        print("\n" + colorize("No files were modified by hooks that need staging.", "1;32"))
        
        # Check if there were any hook failures or syntax errors
        if failed_hooks or syntax_errors or any(arg.lower() == "failed" for arg in sys.argv):
            if not syntax_errors:  # Only show this if we don't have syntax errors (which are more critical)
                print("\n" + colorize("However, some hooks failed. Fix the issues and try again.", "1;31"))
