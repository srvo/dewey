...
def run_final_tests() -> bool:
    ...
    try:
        subprocess.run(
            [sys.executable, "-m", "pytest", "tests/dewey/core/test_script_compliance.py", "-v"],
            check=True,
            text=True
        )
        return True
        ...
        print("Review the output above and the generated files in scripts/non_compliant/")
        return False
...
def main():
    ...
    try:
        ...
        if not run_script(
            ...
            "Update non-compliant files to meet code standards"
        ):
            sys.exit(1)
