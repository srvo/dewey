...
def extract_files(test_output: str) -> Dict[str, List[str]]:
    ...
    patterns = {
        ...
        for match in matches:
            ...
            cleaned_paths = [path.strip().replace('E           ', '') for path in file_list if path.strip() and '/Users/srvo/dewey/' in path]
...
def write_results(non_compliant: Dict[str, List[str]], needed_configs: Dict[str, Set[str]]) -> None:
    ...
    metadata = {
        'summary': {
            category: len(files) for category, files in non_compliant.items()
        },
        'total_unique_files': len(set().union(*[set(files) for files in non_compliant.values()]))
    }
