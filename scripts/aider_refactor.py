...
try:
    ...
    print("Error: aider not installed. Install with: pip install aider-chat", file=sys.stderr)
    sys.exit(1)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dir", required=True, help="Directory or file to process"
    )
    ...
    parser.add_argument(
        "--conventions-file", help="File containing coding conventions"
    )
    parser.add_argument("--verbose", action="store_true", help="Verbose output")
    parser.add_argument(
        "--timeout", type=int, default=60, help="Timeout in seconds for processing each file"
    )
    parser.add_argument(
        "--check-for-urls", action="store_true", help="Check for URLs in prompts"
    )
    parser.add_argument(
        "--custom-prompt", help="Custom prompt for Aider"
    )
    parser.add_argument(
        "--persist-session", action="store_true", help="Use a persistent Aider session"
    )
    parser.add_argument(
        "--session-dir", help="Directory to store persistent session files", default=".aider"
    )
...
def initialize_persistent_session(args):
    ...
    try:
        ...
        logger.info(f"Initializing persistent Aider session with {len(initial_files)} files")
...
def fix_file_with_aider(
    ...
    if file_path.is_dir():
        ...
        logger.warning(f"Skipping directory {file_path} - need a custom prompt to process directories")
        return False
            ...
            try:
                ...
                if hasattr(coder, 'add_files'):
                    ...
                    try:
                        ...
                        if hasattr(coder.repo_map, 'files') and file_str not in coder.repo_map.files:
                            has_file = False
                        # Method 2: Check if the file is in repo_map.get_all_files()
                        elif hasattr(coder.repo_map, 'get_all_files') and file_str not in coder.repo_map.get_all_files():
                            has_file = False
                        # Method 3: Check if the file is directly in repo_map's keys
                        elif hasattr(coder.repo_map, 'keys') and file_str not in list(coder.repo_map.keys()):
                            has_file = False
                        # Method 4: Check if repo_map is a dictionary with the file as a key
                        elif isinstance(coder.repo_map, dict) and file_str not in coder.repo_map:
                            has_file = False
                        else:
                            has_file = True
                    except Exception:
                    ...
                    logger.info(f"Current coder doesn't support add_files, using direct file content for {file_path}")
                    ...
                    if not hasattr(coder, 'repo_map'):
                        logger.info(f"Coder doesn't have a repo_map, assuming WholeFileCoder for {file_path}")
                        ...
                        logger.warning(f"Unexpected coder type - has repo_map but no add_files method for {file_path}")
                ...
                try:
                    logger.info(f"Falling back to creating a new coder instance for {file_path}")
                    null_device = tempfile.mktemp()
                    model = Model(model_name)
                    io = InputOutput(yes=True, input_history_file=null_device)
                    coder = Coder.create(
...
def main():
    ...
    if path.is_dir() and args.custom_prompt:
        ...
        try:
            ...
            logger.info(f"Enhanced prompt with summaries of {len(file_summaries)} files")
    ...
    for file_path in python_files:
        ...
        try:
            ...
            if file_path.is_dir():
                logger.warning(f"Skipping directory {file_path} - only processing individual files")
                continue
