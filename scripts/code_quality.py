...
def run_black(file_path: Path, check_only: bool) -> Tuple[Path, bool, str]:
    ...
    try:
        ...
        return file_path, was_formatted, result.stderr if result.stderr else result.stdout
    except Exception as e:
...
def process_file(
    ...
    result = {"file": file_path, "formatted": False, "issues_before": [], "issues_after": []}
...
def main() -> None:
    ...
    try:
        ...
        logger.error("flake8 and/or black are not installed. Please install them first.")
        sys.exit(1)
    ...
    for file_path in python_files:
        ...
        result = process_file(file_path, args.check_only, args.max_line_length, args.verbose)
    ...
    print(f"Files with issues before: {files_with_issues_before} ({total_issues_before} issues)")
    print(f"Files with issues after: {files_with_issues_after} ({total_issues_after} issues)")
    print(f"Issues fixed: {total_issues_before - total_issues_after}")
    
    if args.verbose:
        print("\nIssues by type before formatting:")
        for code, count in sorted(issues_before_by_code.items(), key=lambda x: x[1], reverse=True):
            print(f"  {code}: {count}")
        
        print("\nIssues by type after formatting:")
        for code, count in sorted(issues_after_by_code.items(), key=lambda x: x[1], reverse=True):
            print(f"  {code}: {count}")
    ...
    if files_with_issues_after > 0:
        ...
        for r in results:
            if r.get("issues_after", []):
                ...
                if args.verbose:
                    for issue in r.get("issues_after", [])[:5]:  # Show first 5 issues only
                        print(f"    {issue}")
                    if len(r.get("issues_after", [])) > 5:
                        print(f"    ... and {len(r.get('issues_after', [])) - 5} more issues")
