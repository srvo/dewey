...
def read_precommit_output():
    ...
    if sys.stdin.isatty():
        print("Error: No input piped to script. This should be run as part of pre-commit.")
        return None
...
def extract_issues(output):
    ...
    class_issue_pattern = re.compile(r"Class '([^']+)' inherits from 'BaseScript' but doesn't implement the required '([^']+)' method")
    ...
    for line in output.split('\n'):
        ...
        if class_match:
            ...
            if file_path:
                ...
                issues_by_file[file_path].append(f"Class '{class_name}' needs to implement '{method_name}' method (required by BaseScript)")
            else:
                hook_issues.append(f"Class '{class_name}' needs to implement '{method_name}' method (in unknown file)")
    
    # Extract hook failures (excluding those that automatically fixed issues)
    hook_failure_pattern = re.compile(r'([^\n.]+)\.+Failed\n([^.]+?)(?=\n[^\s]+\.\.\.|$)', re.DOTALL)
    for match in hook_failure_pattern.finditer(output):
        ...
        if failure_details and not failure_details.startswith("Fixing"):
            ...
            file_match = re.search(r'([^\s:]+\.(py|js|json|yaml|yml|md))', failure_details)
            ...
            if file_match:
                ...
                issues_by_file[file_path].append(f"{hook_name} failure: {failure_details.split('Fixing')[0].strip()}")
            else:
                hook_issues.append(f"Fix `{hook_name}` failure: {failure_details.split('Fixing')[0].strip()}")
...
def update_todo_file(issues):
    ...
    if section_header in content:
        ...
        pattern = re.compile(f"{re.escape(section_header)}.*?(?=^##|$)", re.MULTILINE | re.DOTALL)
