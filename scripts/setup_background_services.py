...
def setup_unified_processor_systemd():
    ...
    try:
        ...
        logger.info("To start the service, run: sudo systemctl start dewey-unified-processor")
...
def setup_unified_processor_launchd():
    ...
    try:
        ...
        # Get paths
        processor_script = project_root / "src" / "dewey" / "core" / "crm" / "gmail" / "run_unified_processor.py"
...
"""
        ...
        logger.info(f"LaunchAgent set up successfully at {plist_path}")
        logger.info("The unified processor will start automatically at boot and restart if it fails")
