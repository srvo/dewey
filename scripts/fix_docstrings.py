...
def fix_docstrings(file_path: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        fixed = (linter_result.returncode == 0 and "1 fixed" in linter_result.stderr) or \
                (formatter_result.returncode == 0)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fix docstring formatting issues using Ruff")
