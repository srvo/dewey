            ...
            if import_match:
                ...
                for part in re.split(r',\s*', imports):
                    if ' as ' in part:
                        ...
                        if (module, part.strip(), part.strip()) not in unused_from_imports:
                            parts.append(part)
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.ClassDef, ast.AsyncFunctionDef)):
                if not ast.get_docstring(node):
                    missing_docstrings.append((node.lineno, node.name, isinstance(node, ast.ClassDef)))
...
def fix_mutable_defaults(content: str) -> str:
    ...
    def replace_mutable_defaults(match):
        ...
        for param in re.split(r',\s*', params):
            if '=' in param and any(m in param for m in ('[]', '{}', 'list()', 'dict()', 'set()')):
...
def fix_file(file_path: Path, dry_run: bool = False) -> Dict:
    ...
    try:
        ...
        return {
            "file": file_path, "changes": changes, "modified": bool(changes), }
    except Exception as e:
        logger.error(f"Error fixing {file_path}: {e}")
        return {
            "file": file_path, "changes": [], "modified": False, "error": str(e), }
...
def main():
    ...
    parser.add_argument("--dir", "-d", required=True, help="File or directory to process")
    parser.add_argument("--dry-run", action="store_true", help="Don't actually change files")
