...
def run_bidirectional_sync(execute_alters=False, force_imports=False, add_primary_key=True):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run bidirectional schema synchronization.")
    parser.add_argument("--execute", action="store_true", 
                      help="Execute ALTER statements to update database schema")
    parser.add_argument("--force-imports", action="store_true", 
                      help="Force adding imports even if they might already exist")
    parser.add_argument("--no-primary-key", action="store_false", dest="add_primary_key", 
                      help="Disable adding virtual primary keys for tables without them")
    
    args = parser.parse_args()
    
    sys.exit(run_bidirectional_sync(
