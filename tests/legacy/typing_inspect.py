...
# Refactored from: typing_inspect
# Date: 2025-03-16T16:19:11.534320
...
if NEW_TYPING:
    ...
    try:
        from typing import _ClassVar, _Union
    except ImportError:
...
def is_tuple_type(tp):
    ...
    if NEW_TYPING:
        return (
            tp is tuple
            or (isinstance(tp, typingGenericAlias) and tp.__origin__ is tuple)
            or (
                isinstance(tp, type)
                and issubclass(tp, Generic)
...
def get_last_origin(tp):
    ...
    """
    if NEW_TYPING:
        msg = "This function is only supported in Python 3.6, use get_origin instead"
        raise ValueError(
            msg,
        )
    sentinel = object()
...
def get_parameters(tp):
    ...
    if LEGACY_TYPING:
        ...
        return ()
    if NEW_TYPING:
        if (isinstance(tp, typingGenericAlias) and hasattr(tp, "__parameters__")) or (
            isinstance(tp, type) and issubclass(tp, Generic) and tp is not Generic
        ):
...
def get_last_args(tp):
    ...
    """
    if NEW_TYPING:
        msg = "This function is only supported in Python 3.6, use get_args instead"
        raise ValueError(
            msg,
        )
    if is_classvar(tp):
...
def get_args(tp, evaluate=None):
    ...
    if NEW_TYPING:
        if evaluate is not None and not evaluate:
            msg = "evaluate can only be True in Python >= 3.7"
...
def _subs_tree(cls, tvars=None, args=None):
    ...
    if current is None and not is_union_type(cls) and not is_tuple_type(cls):
        return cls
...
def _union_has_type_var(tp) -> bool:
...
def _tuple_has_type_var(tp) -> bool:
...
def _generic_has_type_var(tp) -> bool:
