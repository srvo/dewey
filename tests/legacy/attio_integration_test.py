...
# Refactored from: attio_integration_test
# Date: 2025-03-16T16:19:09.949584
# Refactor Version: 1.0
# Formatting failed: LLM generation failed: Gemini API error: Invalid operation: The `response.text` quick accessor requires the response to contain a valid `Part`, but none were returned. The candidate's [finish_reason](https://ai.google.dev/api/generate-content#finishreason) is 4. Meaning that the model was reciting from copyrighted material.
...
def main() -> None:
    ...
    try:
        ...
        logger.exception(f"API request failed: {e!s}")
