...
def get_current_universe() -> List[Dict[str, str]]:
...
def analyze_financial_changes(ticker: str) -> List[Dict[str, Any]]:
...
def analyze_material_events(ticker: str) -> List[str]:
    ...
    changes = analyze_financial_changes(ticker)
...
def main() -> None:
    try:
        ...
        for stock in stocks:
            ...
            events = analyze_material_events(ticker)
            if events:
            ...
            for sector, findings in by_sector.items():
                for finding in findings:
                    for event in finding["events"]:
                        logger.info(f"{finding['ticker']} ({sector}): {event}")
        else:
            logger.info("No material findings to report")
