...
def ensure_table_exists(conn: Any, table_name: str, schema_sql: str) -> None:
...
def initialize_database(
    ...
    if existing_db_path and os.path.exists(existing_db_path):
        ...
        try:
            with db_manager.get_connection(for_write=True) as conn:
                return conn
        except Exception as e:
            logger.warning(f"Failed to connect to existing database: {e}")
...
def get_table_info(table_name: str, local_only: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        indexes = db_manager.execute_query(
            f"PRAGMA show_tables LIKE '{table_name}_idx%'",
            local_only=local_only
        )
...
def backup_table(table_name: str, backup_dir: str) -> str:
    ...
    try:
        ...
        backup_file = os.path.join(
            backup_dir,
            f"{table_name}_backup_{timestamp}.parquet"
        )
        ...
        logger.info(f"Created backup of {table_name} at {backup_file}")
        return backup_file
...
def restore_table(table_name: str, backup_file: str) -> None:
    ...
    try:
        ...
        logger.info(f"Restored {table_name} from {backup_file}")
        
    except Exception as e:
        logger.error(f"Error restoring table {table_name}: {e}")
        raise

def vacuum_database(local_only: bool = True) -> None:
...
def analyze_table(table_name: str, local_only: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        info = get_table_info(table_name, local_only)
...
def sync_tables(source_table: str, target_table: str,
                key_column: str, local_only: bool = False) -> int:
    ...
    try:
        ...
        try:
            ...
            updated = db_manager.execute_query(f"""
            ...
            # Insert new records
            inserted = db_manager.execute_query(f"""
