...
def sample_json_file(tmp_path):
    ...
    data = {
        "meta": {"timestamp": "2023-01-01T00:00:00", "version": "1.0"},
        "companies": [
            {
...
def test_process_risk_factors_update(tmp_duckdb, tmp_path):
    ...
    initial_count = conn.execute(
        "SELECT COUNT(*) FROM company_risk_factors"
    ).fetchone()[0]
    assert initial_count == 1

    # Re-process with new data
    process_risk_factors(conn, company_id, {"key_risks": ["New Risk"]})
    final_count = conn.execute("SELECT COUNT(*) FROM company_risk_factors").fetchone()[
        0
    ]
