...
from dewey.core.tui.workers import AnalysisWorker, UploadWorker, ConfigWorker
...
async def test_upload_worker(pilot: Pilot[DeweyTUI], sample_csv_file: Path):
    ...
    await simulate_file_upload(
        pilot,
        str(sample_csv_file.parent),
        "test_db"
    )
