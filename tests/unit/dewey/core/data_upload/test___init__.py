...
class TestDataUploader:
    ...
    def test_run_method_no_api_key(
        self, data_uploader: DataUploader, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_run_method_with_api_key_db_and_llm(
        self, data_uploader: DataUploader, caplog: pytest.LogCaptureFixture
    ) -> None:
        ...
        data_uploader.llm_client.generate_text = MagicMock(
            return_value="Test LLM Response"
        )
    ...
    def test_run_method_exception(
        ...
        data_uploader.get_config_value = MagicMock(
            side_effect=Exception("Test Exception")
        )
        with caplog.records_property() as records:
            data_uploader.run()
            assert (
                "An error occurred during data upload: Test Exception"
    ...
    def test_cleanup_method_exception(
        self, data_uploader: DataUploader, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_get_config_value_nested_missing_key(
    ...
    def test_load_config_specific_section(
        self, data_uploader: DataUploader, tmp_path: Path
    ) -> None:
    ...
    def test_initialize_db_connection_success(
        self, mock_get_connection: MagicMock, data_uploader: DataUploader
    ) -> None:
    ...
    @patch(
        "dewey.core.data_upload.get_connection", side_effect=Exception("Test Exception")
    )
    def test_initialize_db_connection_exception(
        ...
        data_uploader.config = {"core": {"database": {"test": "config"}}}
        with pytest.raises(
            Exception, match="Failed to initialize database connection: Test Exception"
        ):
            data_uploader._initialize_db_connection()
        assert data_uploader.db_conn is None
    ...
    def test_initialize_llm_client_success(
        self, mock_get_llm_client: MagicMock, data_uploader: DataUploader
    ) -> None:
    ...
    @patch(
        "dewey.core.data_upload.get_llm_client", side_effect=Exception("Test Exception")
    )
    def test_initialize_llm_client_exception(
        ...
        data_uploader.config = {"llm": {"test": "config"}}
        with pytest.raises(
            Exception, match="Failed to initialize LLM client: Test Exception"
        ):
            data_uploader._initialize_llm_client()
        assert data_uploader.llm_client is None
    ...
    def test_parse_args_log_level(
        self, mock_parse_args: MagicMock, data_uploader: DataUploader
    ) -> None:
    ...
    def test_parse_args_config(
        self, mock_parse_args: MagicMock, data_uploader: DataUploader, tmp_path: Path
    ) -> None:
        ...
        data_uploader.logger.info.assert_called_once_with(
    ...
    def test_parse_args_config_not_found(
        ...
        assert exc_info.value.code == 1
        data_uploader.logger.error.assert_called_once_with(
