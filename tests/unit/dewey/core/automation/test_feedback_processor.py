...
class TestFeedbackProcessor:
    ...
    def test_init(self, feedback_processor: FeedbackProcessor) -> None:
        ...
        assert (
            feedback_processor.description
        ...
        assert (
            feedback_processor.db_file
            == f"{feedback_processor.active_data_dir}/process_feedback.duckdb"
        )
        assert (
            feedback_processor.classifier_db
            == f"{feedback_processor.active_data_dir}/email_classifier.duckdb"
        )

    @patch("duckdb.connect")
    def test_init_db_success(
        self, mock_connect: MagicMock, feedback_processor: FeedbackProcessor
    ) -> None:
    ...
    @patch("duckdb.connect")
    def test_init_db_conflicting_lock_retry(
        self, mock_connect: MagicMock, feedback_processor: FeedbackProcessor
    ) -> None:
    ...
    @patch("duckdb.connect")
    def test_init_db_conflicting_lock_fail(
        self, mock_connect: MagicMock, feedback_processor: FeedbackProcessor
    ) -> None:
    ...
    @patch("duckdb.connect")
    def test_init_db_other_exception(
        self, mock_connect: MagicMock, feedback_processor: FeedbackProcessor
    ) -> None:
    ...
    def test_load_preferences_no_result(
        self, feedback_processor: FeedbackProcessor, mock_db_connection: MagicMock
    ) -> None:
    ...
    def test_save_feedback(
        ...
        assert (
            mock_db_connection.execute.call_count == len(sample_feedback_data) + 2
        )  # +2 for BEGIN/COMMIT TRANSACTION
    ...
    def test_generate_feedback_json_success(
        self, mock_generate_json: MagicMock, feedback_processor: FeedbackProcessor
    ) -> None:
        ...
        result = feedback_processor.generate_feedback_json(
            "Test feedback", "123", "Test", 3
        )
    ...
    def test_generate_feedback_json_unsubscribe(
        ...
        result = feedback_processor.generate_feedback_json(
            "Please unsubscribe me", "123", "Test", 3
        )
    ...
    @patch("dewey.core.automation.feedback_processor.generate_json")
    def test_generate_feedback_json_api_error(
        self, mock_generate_json: MagicMock, feedback_processor: FeedbackProcessor
    ) -> None:
    ...
    @patch("dewey.core.automation.feedback_processor.generate_json")
    def test_generate_feedback_json_invalid_json(
        self, mock_generate_json: MagicMock, feedback_processor: FeedbackProcessor
    ) -> None:
    ...
    def test_suggest_rule_changes_insufficient_data(
        self,
        feedback_processor: FeedbackProcessor,
        sample_feedback_data: List[Dict[str, Any]],
        sample_preferences: Dict[str, Any],
        ...
        result = feedback_processor.suggest_rule_changes(
            sample_feedback_data[:2], sample_preferences
        )

        assert result == []
        feedback_processor.logger.info.assert_called_with(
            "Not enough feedback data to suggest changes."
        )
    ...
    def test_suggest_rule_changes_no_discrepancies(
        ...
        result = feedback_processor.suggest_rule_changes(
            sample_feedback_data, sample_preferences
        )

        assert result == []

    def test_suggest_rule_changes_topic_suggestions(
        self,
        feedback_processor: FeedbackProcessor,
        sample_feedback_data: List[Dict[str, Any]],
        sample_preferences: Dict[str, Any],
    ...
    def test_suggest_rule_changes_source_suggestions(
        self,
        feedback_processor: FeedbackProcessor,
        sample_feedback_data: List[Dict[str, Any]],
        sample_preferences: Dict[str, Any],
    ...
    def test_update_preferences_add_override_rule(
        self, feedback_processor: FeedbackProcessor, sample_preferences: Dict[str, Any]
    ) -> None:
        ...
        updated_preferences = feedback_processor.update_preferences(
            sample_preferences, changes
        )

        assert len(updated_preferences["override_rules"]) == 2
        assert {"keywords": ["new_topic"], "min_priority": 2} in updated_preferences[
            "override_rules"
        ]

    def test_update_preferences_adjust_weight(
        self, feedback_processor: FeedbackProcessor, sample_preferences: Dict[str, Any]
    ) -> None:
    ...
    def test_run_success(
        ...
        mock_suggest_rule_changes.return_value = [
            {
                "type": "add_override_rule",
                "keyword": "test",
                "priority": 1,
        ...
        mock_suggest_rule_changes.assert_called_once_with(
            sample_feedback_data, sample_preferences
        )
        ...
        mock_save_feedback.assert_called_once_with(
            mock_init_db.return_value, sample_feedback_data
        )
    ...
    def test_run_no_feedback(
        ...
        mock_load_feedback.assert_called_once()
        feedback_processor.logger.info.assert_called_with(
            "No feedback data available to process."
        )
    ...
    def test_execute_keyboard_interrupt(
        ...
        mock_parse_args.assert_called_once()
        mock_run.assert_called_once()
        feedback_processor.logger.warning.assert_called_once_with(
