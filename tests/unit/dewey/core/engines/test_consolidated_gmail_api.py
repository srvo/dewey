...
class TestConsolidatedGmailApi:
    ...
    @patch("dewey.core.engines.consolidated_gmail_api.ConsolidatedGmailApi.get_config_value")
    def test_run_api_key_present(
        self, mock_get_config_value: MagicMock, consolidated_gmail_api: ConsolidatedGmailApi
    ) -> None:
        ...
        consolidated_gmail_api.logger.info.assert_called_with("Consolidated Gmail API script finished")
        consolidated_gmail_api.logger.debug.assert_called_with("API key loaded from config")
    ...
    def test_run_api_key_absent(
        ...
        mock_get_config_value.assert_called_once_with("api_key")
        consolidated_gmail_api.logger.info.assert_called_with("Consolidated Gmail API script finished")
        consolidated_gmail_api.logger.warning.assert_called_with("API key not found in config")

    def test_run_script_logic(self, consolidated_gmail_api: ConsolidatedGmailApi) -> None:
