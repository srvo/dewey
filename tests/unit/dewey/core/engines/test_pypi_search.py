...
def mock_base_script(monkeypatch: pytest.MonkeyPatch) -> MockBaseScript:
    ...
    monkeypatch.setattr(
        BaseScript, "_load_config", lambda self: {"package_name": "requests"}
    )
...
def test_pypi_search_run_success(
    pypi_search: PypiSearch, caplog: pytest.LogCaptureFixture
) -> None:
...
def test_pypi_search_run_exception(
    pypi_search: PypiSearch, caplog: pytest.LogCaptureFixture
) -> None:
    """Test that PypiSearch.run() handles exceptions correctly."""
    caplog.set_level(logging.ERROR)
    with patch.object(
        pypi_search, "get_config_value", side_effect=Exception("Config error")
    ):
        pypi_search.run()
...
def test_pypi_search_get_config_value(
    pypi_search: PypiSearch, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_pypi_search_get_config_value_nested(
    pypi_search: PypiSearch, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Test that PypiSearch.get_config_value() retrieves nested config values correctly."""
    monkeypatch.setattr(
        pypi_search, "config", {"nested": {"package_name": "sqlalchemy"}}
    )
...
def test_pypi_search_get_config_value_missing_intermediate(
