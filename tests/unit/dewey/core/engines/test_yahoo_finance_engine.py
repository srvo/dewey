...
class TestYahooFinanceEngine:
    ...
    @patch("dewey.core.engines.yahoo_finance_engine.YahooFinanceEngine.get_config_value")
    def test_run_success(
        self, mock_get_config_value: pytest.fixture, yahoo_finance_engine: YahooFinanceEngine
    ) -> None:
    ...
    def test_run_no_api_key(
        ...
        with patch.object(yahoo_finance_engine.logger, "error") as mock_logger_error:
            yahoo_finance_engine.run()
            mock_logger_error.assert_called_once_with("API key not found in configuration.")
            mock_get_config_value.assert_called_once_with("api_key")
