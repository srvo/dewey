...
@pytest.fixture
def consolidate_databases_instance(
    mock_config: Dict[str, Any], tmp_path: Path
...
def test_consolidate_databases_initialization(
    consolidate_databases_instance: ConsolidateDatabases,
) -> None:
    """
...
def test_consolidate_databases_run_success(
    mock_get_connection: Any, consolidate_databases_instance: ConsolidateDatabases
) -> None:
    """
...
@patch("dewey.core.db.consolidate_databases.get_connection")
def test_consolidate_databases_run_failure(
    mock_get_connection: Any, consolidate_databases_instance: ConsolidateDatabases
) -> None:
    """
...
def test_consolidate_databases_run_no_config(
    consolidate_databases_instance: ConsolidateDatabases, tmp_path: Path
) -> None:
    """
    ...
    with patch("dewey.core.base_script.CONFIG_PATH", tmp_path / "dewey.yaml"):
        consolidator = ConsolidateDatabases()
        with pytest.raises(
            ValueError, match="Source or target database URL not configured."
        ):
            consolidator.run()
...
def test_consolidate_databases_execute_success(
    mock_run: Any,
    mock_parse_args: Any,
    consolidate_databases_instance: ConsolidateDatabases,
) -> None:
...
@patch("dewey.core.db.consolidate_databases.ConsolidateDatabases.parse_args")
@patch("dewey.core.db.consolidate_databases.ConsolidateDatabases.run")
def test_consolidate_databases_execute_keyboard_interrupt(
    mock_run: Any,
    mock_parse_args: Any,
    consolidate_databases_instance: ConsolidateDatabases,
...
@patch("dewey.core.db.consolidate_databases.ConsolidateDatabases.parse_args")
@patch("dewey.core.db.consolidate_databases.ConsolidateDatabases.run")
def test_consolidate_databases_execute_exception(
    mock_run: Any,
    mock_parse_args: Any,
    consolidate_databases_instance: ConsolidateDatabases,
    capsys: pytest.CaptureFixture,
...
@patch("dewey.core.db.consolidate_databases.get_connection")
def test_consolidate_databases_cleanup(
    mock_get_connection: Any, consolidate_databases_instance: ConsolidateDatabases
) -> None:
    """
...
def test_consolidate_databases_get_config_value(
    consolidate_databases_instance: ConsolidateDatabases,
) -> None:
    """
    ...
    assert value == "INFO"
    default_value = consolidate_databases_instance.get_config_value(
        "nonexistent.key", "default"
    )
    assert default_value == "default"


def test_consolidate_databases_get_path(
    consolidate_databases_instance: ConsolidateDatabases,
) -> None:
    """
    ...
    assert str(relative_path).endswith(
        "config/dewey.yaml"
    )  # Adjust assertion as needed
