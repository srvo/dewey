...
def mock_config() -> Dict[str, Any]:
    ...
    return {
        "architecture": {"example_config": "test_value"},
        "core": {
            "logging": {
                "level": "DEBUG",
...
def mock_base_script(mock_config: Dict[str, Any]) -> MagicMock:
    ...
    with patch(
        "dewey.core.architecture.BaseScript.__init__", return_value=None
    ) as MockBaseScript:
...
def test_run_method_success(
    ...
    architecture_module.get_config_value.assert_called_with(
        "example_config", default="default_value"
    )


def test_run_method_no_db_connection(
    architecture_module: ArchitectureModule,
    mock_config: Dict[str, Any],
    caplog: pytest.LogCaptureFixture,
) -> None:
...
def test_run_method_no_llm_client(
    architecture_module: ArchitectureModule,
    mock_config: Dict[str, Any],
    caplog: pytest.LogCaptureFixture,
) -> None:
...
def test_run_method_exception(
    ...
    architecture_module.get_config_value = MagicMock(
    ...
    assert (
        "An error occurred during architecture module execution: Test error"
        in caplog.text
    )
...
def test_load_config_file_not_found(
    architecture_module: ArchitectureModule, caplog: pytest.LogCaptureFixture
) -> None:
...
def test_load_config_specific_section(
    architecture_module: ArchitectureModule, mock_config: Dict[str, Any], tmp_path: Path
) -> None:
...
def test_load_config_specific_section_not_found(
    architecture_module: ArchitectureModule,
    mock_config: Dict[str, Any],
    tmp_path: Path,
    caplog: pytest.LogCaptureFixture,
    ...
    with patch("dewey.core.architecture.CONFIG_PATH", new=config_file):
        ...
        assert (
            "Config section 'nonexistent_section' not found in dewey.yaml. Using full config."
            in caplog.text
        )


def test_get_config_value_existing_key(
    architecture_module: ArchitectureModule, mock_config: Dict[str, Any]
) -> None:
...
def test_get_config_value_default_value(
    ...
    architecture_module.config = mock_config["architecture"]
    value = architecture_module.get_config_value(
...
def test_get_config_value_nested_key_missing(
    architecture_module: ArchitectureModule, mock_config: Dict[str, Any]
) -> None:
    """Test getting a nested configuration value that is missing."""
    architecture_module.config = mock_config
    value = architecture_module.get_config_value(
        "core.nonexistent_key", default="default_value"
    )
    assert value == "default_value"
...
def test_setup_logging_default_config(
    ...
    caplog.set_level(logging.INFO)
    with patch(
        "dewey.core.architecture.CONFIG_PATH", new=tmp_path / "nonexistent_config.yaml"
    ):
        architecture_module._setup_logging()
...
def test_parse_args_db_connection_string(
    ...
    with patch(
        "sys.argv", ["script_name", "--db-connection-string", "test_connection_string"]
    ):
        args = architecture_module.parse_args()
