...
class TestFinancialPipeline:
    ...
    def test_run_exception(
        ...
        assert "Starting financial analysis pipeline..." in caplog.text
        assert (
            "Test exception" not in caplog.text
        )  # Exception should be caught in BaseScript.execute
    ...
    def test_get_config_value_existing_key(
        self, financial_pipeline: FinancialPipeline
    ) -> None:
    ...
    def test_get_path_absolute_path(
        self, financial_pipeline: FinancialPipeline
    ) -> None:
    ...
    def test_get_path_relative_path(
        self, financial_pipeline: FinancialPipeline
    ) -> None:
    ...
    def test_parse_args_log_level(
        self, mock_parse_args: pytest.fixture, financial_pipeline: FinancialPipeline
    ) -> None:
    ...
    def test_parse_args_db_connection_string(
        ...
        with patch(
            "dewey.core.research.analysis.financial_pipeline.get_connection"
        ) as mock_get_connection:
            financial_pipeline.parse_args()
            mock_get_connection.assert_called_once_with(
                {"connection_string": "test_db_string"}
            )
    ...
    def test_parse_args_llm_model(
        ...
        with patch(
            "dewey.core.research.analysis.financial_pipeline.get_llm_client"
    ...
    def test_cleanup_db_connection(
        self, mock_get_connection: pytest.fixture, financial_pipeline: FinancialPipeline
    ) -> None:
