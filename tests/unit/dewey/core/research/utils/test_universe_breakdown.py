...
def mock_duckdb_connection(
    mock_universe_file: Path, tmp_path: Path
...
def mock_env_variables(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, mock_universe_file: Path
) -> None:
...
def test_sector_breakdown(
    mock_duckdb_connection: duckdb.DuckDBPyConnection, mock_universe_file: Path
) -> None:
    """Test the sector breakdown query."""
    expected_df = pd.DataFrame({"sector": ["Technology"], "count": [3]})
    result_df = mock_duckdb_connection.execute(
        f"""
...
def test_main_functionality(
    mock_con, mock_universe, mock_universe_file, tmp_path, monkeypatch
):
...
def test_duckdb_connection_error(
