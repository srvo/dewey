...
class TestResearchOutputHandler:
    ...
    @patch(
        "dewey.core.research.utils.research_output_handler.open", new_callable=mock_open
    )
    def test_save_output_file(
        self, mock_file: mock_open, research_output_handler: ResearchOutputHandler
    ) -> None:
        ...
        # Check log message
        assert (
            "Research output saved to: test_output.txt"
            in research_output_handler.logger.handlers[0].format(
    ...
    @patch(
        "dewey.core.research.utils.research_output_handler.open",
        side_effect=Exception("File error"),
    )
    def test_save_output_file_error(
        self, mock_file: mock_open, research_output_handler: ResearchOutputHandler
    ) -> None:
        ...
        # Check log message
        assert (
            "Error saving research output: File error"
            in research_output_handler.logger.handlers[0].format(
                research_output_handler.logger.handlers[0].record_factory(
                    "ERROR", 40, "", 0, "", (), None
                )
    ...
    def test_save_output_db_no_connection(
        self, research_output_handler: ResearchOutputHandler
    ) -> None:
    ...
    def test_run_success(
        ...
        # Check log messages
        assert (
            "Starting research output handling..."
            in research_output_handler.logger.handlers[0].format(
                research_output_handler.logger.handlers[0].record_factory(
                    "INFO", 20, "", 0, "", (), None
                )
        ...
        assert (
            "Research output handling completed successfully."
            in research_output_handler.logger.handlers[0].format(
                research_output_handler.logger.handlers[0].record_factory(
                    "INFO", 20, "", 0, "", (), None
                )
    ...
    @patch(
        "dewey.core.research.utils.research_output_handler.ResearchOutputHandler.save_output",
        side_effect=Exception("Save error"),
    )
    def test_run_error(
        self,
        mock_save_output: mock_open,
        research_output_handler: ResearchOutputHandler,
    ) -> None:
        ...
        assert (
            "An error occurred during research output handling: Save error"
            in research_output_handler.logger.handlers[0].format(
                research_output_handler.logger.handlers[0].record_factory(
                    "ERROR", 40, "", 0, "", (), None
                )
    ...
    def test_get_config_value(
        ...
        assert (
            research_output_handler.get_config_value("section1.key4", "default_value")
            == "default_value"
        )
        ...
        assert (
            research_output_handler.get_config_value("nested.level1.level2")
            == "deep_value"
        )
