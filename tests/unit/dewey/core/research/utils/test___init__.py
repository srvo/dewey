...
class TestResearchUtils:
    ...
    def test_init(self, research_utils: ResearchUtils) -> None:
        ...
        assert research_utils.description == "Provides utility functions for research workflows."
    ...
    def test_run(self, mock_get_config_value: MagicMock, research_utils: ResearchUtils, caplog: pytest.LogCaptureFixture) -> None:
        """Tests the run method."""
        mock_get_config_value.return_value = "test_value"
        with caplog.record_tuples():
            caplog.set_level(logging.INFO)
            research_utils.run()
        assert ("dewey.core.research.utils", logging.INFO, "Starting ResearchUtils...") in caplog.record_tuples()
        assert ("dewey.core.research.utils", logging.INFO, "Example config value: test_value") in caplog.record_tuples()
        assert ("dewey.core.research.utils", logging.INFO, "Executing example utility function...") in caplog.record_tuples()
        assert ("dewey.core.research.utils", logging.INFO, "Example utility function completed.") in caplog.record_tuples()
        assert ("dewey.core.research.utils", logging.INFO, "ResearchUtils completed.") in caplog.record_tuples()
    ...
    def test__example_utility_function(self, mock_logger: MagicMock, research_utils: ResearchUtils) -> None:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("valid_source", "some_data", "some_data", logging.INFO, "Successfully retrieved data from valid_source."),
            ("empty_source", None, None, logging.WARNING, "No data found for data source: empty_source."),
        ],
    )
    @patch("dewey.core.research.utils.ResearchUtils.get_config_value")
    ...
    @patch("dewey.core.research.utils.ResearchUtils.get_config_value")
    def test_get_data_error(self, mock_get_config_value: MagicMock, research_utils: ResearchUtils, caplog: pytest.LogCaptureFixture) -> None:
        ...
        assert ("dewey.core.research.utils", logging.ERROR, "Error retrieving data from error_source: Simulated error") in caplog.record_tuples()
