...
class TestResearchWorkflow:
    ...
    def test_run(
        self, research_workflow: ResearchWorkflow, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_get_config_value(self, research_workflow: ResearchWorkflow) -> None:
        ...
        value = research_workflow.get_config_value(
        ...
        value = research_workflow.get_config_value(
            "non_existent.test_key", "default_value"
        )
        assert value == "default_value"

    def test_get_config_value_no_config(
    ...
    def test_get_config_value_empty_config(
        self, research_workflow: ResearchWorkflow
    ) -> None:
    ...
    def test_inheritance_from_base_script(
        self, research_workflow: ResearchWorkflow
    ) -> None:
    ...
    def test_run_with_mocked_logger(
        self, mock_logger, research_workflow: ResearchWorkflow
    ) -> None:
        """Tests the run method with a mocked logger."""
        research_workflow.run()
        mock_logger.info.assert_called_once_with(
            f"Running research workflow: {research_workflow.name}"
        )
