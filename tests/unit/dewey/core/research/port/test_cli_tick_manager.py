...
class TestCliTickManager:
    ...
    def cli_tick_manager(self):
        ...
        with patch(
            "dewey.core.research.port.cli_tick_manager.BaseScript.__init__"
        ) as mock_init:
            mock_init.return_value = None
            cli_tick_manager = CliTickManager()
            cli_tick_manager.logger = logging.getLogger(__name__)  # Mock logger
            cli_tick_manager.config = {
                "cli_tick_manager": {"tick_interval": 60}
            }  # Mock config
    ...
    def test_run_custom_tick_interval(self):
        ...
        with patch(
            ...
            cli_tick_manager.logger = logging.getLogger(__name__)  # Mock logger
            cli_tick_manager.config = {
                "cli_tick_manager": {"tick_interval": 120}
            }  # Mock config
    ...
    def test_run_exception_handling(self, cli_tick_manager, caplog):
        ...
        with patch.object(
            cli_tick_manager, "get_config_value", side_effect=Exception("Config error")
        ):
            cli_tick_manager.run()
