...
class TestBookkeepingScript:
    ...
    def test_bookkeeping_script_initialization(self, mock_base_script: MagicMock) -> None:
    ...
    def test_bookkeeping_script_initialization_with_custom_config(self, mock_base_script: MagicMock) -> None:
    ...
    def test_run_method_raises_not_implemented_error(self, bookkeeping_script: BookkeepingScript) -> None:
        """Tests that the run method raises a NotImplementedError."""
        with pytest.raises(NotImplementedError, match="Subclasses must implement the run method."):
            bookkeeping_script.run()

    def test_bookkeeping_script_inherits_from_base_script(self, bookkeeping_script: BookkeepingScript) -> None:
        """Tests that BookkeepingScript inherits from BaseScript."""
        assert isinstance(bookkeeping_script, BaseScript)

    def test_bookkeeping_script_logging(self, bookkeeping_script: BookkeepingScript, caplog: pytest.LogCaptureFixture) -> None:
    ...
    def test_bookkeeping_script_loads_dotenv(self, mock_load_dotenv: MagicMock, bookkeeping_script: BookkeepingScript) -> None:
        """Tests that the BookkeepingScript loads the .env file."""
        mock_load_dotenv.assert_called_once()

    @patch("dewey.core.bookkeeping.BaseScript._load_config")
    def test_bookkeeping_script_loads_config(self, mock_load_config: MagicMock, bookkeeping_script: BookkeepingScript) -> None:
    ...
    def test_bookkeeping_script_initializes_db_connection(self, mock_initialize_db_connection: MagicMock, bookkeeping_script: BookkeepingScript) -> None:
    ...
    def test_bookkeeping_script_does_not_initialize_llm_client(self, mock_initialize_llm_client: MagicMock, bookkeeping_script: BookkeepingScript) -> None:
    ...
    @patch("dewey.core.bookkeeping.BaseScript._initialize_llm_client")
    def test_bookkeeping_script_initializes_llm_client_when_enabled(self, mock_initialize_llm_client: MagicMock, mock_base_script: MagicMock) -> None:
