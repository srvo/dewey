...
class TestMercuryImporter:
    ...
    @pytest.fixture
    def mercury_importer(self, mock_config: MagicMock) -> MercuryImporter:
        """Fixture to create a MercuryImporter instance."""
        importer = MercuryImporter(config=mock_config)
    ...
    def test_init_with_config_db_llm(self, mock_dewey_config: MagicMock) -> None:
        """Test that the MercuryImporter is initialized correctly with config, db_conn, and llm_client."""
        mock_db_conn = MagicMock()
        mock_llm_client = MagicMock()
        importer = MercuryImporter(
            config=mock_dewey_config, db_conn=mock_db_conn, llm_client=mock_llm_client
        )
        assert importer.config == mock_dewey_config
        assert importer.db_conn == mock_db_conn
        assert importer.llm_client == mock_llm_client
    ...
    def test_run_api_key_found(
        ...
        assert "Running Mercury importer" in str(mercury_importer.logger.info.call_args)
        assert "Mercury API key found." in str(mercury_importer.logger.info.call_args)
    ...
    def test_run_api_key_not_found(
        ...
        assert "Running Mercury importer" in str(mercury_importer.logger.info.call_args)
        assert (
            "Mercury API key not found in configuration."
            in str(mercury_importer.logger.warning.call_args)
        )
        mock_get_config_value.assert_called_once_with("api_key")
    ...
    def test_run_db_connection_available(
        ...
        mercury_importer.run()
        assert "Database connection available." in str(mercury_importer.logger.info.call_args)
    ...
    def test_run_db_connection_unavailable(
        ...
        mercury_importer.run()
        assert "No database connection configured." in str(mercury_importer.logger.warning.call_args)
    ...
    def test_run_llm_client_available(
        ...
        mercury_importer.run()
        assert "LLM client available." in str(mercury_importer.logger.info.call_args)
    ...
    def test_run_llm_client_unavailable(
        ...
        mercury_importer.run()
        assert "No LLM client configured." in str(mercury_importer.logger.warning.call_args)
    ...
    def test_run_exception(
        ...
        with pytest.raises(Exception, match="Test exception"):
            mercury_importer.run()
        assert "Error during Mercury import: Test exception" in str(
            mercury_importer.logger.error.call_args
        )
