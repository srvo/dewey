...
def test_database(tmp_path: Path) -> Path:
    ...
    cursor.execute(
        ...
        CREATE TABLE episodes (
            title TEXT, file TEXT, transcript TEXT, link TEXT, publish_date TEXT
        )
    """
    )
    cursor.execute(
...
def test_match_transcript_files_success(
    mock_connect: MagicMock, mock_listdir: MagicMock, transcript_matcher: TranscriptMatcher, mock_config: MagicMock, test_database: Path, test_transcript_directory: Path, ) -> None:
    """Test successful matching of transcript files to episodes."""
    mock_config.side_effect = [
        str(test_database), str(test_transcript_directory), "episodes", "title", "file", "transcript", "link", "publish_date", "utf-8", 0.7, 5, 5, True, ]
    mock_listdir.return_value = ["test_episode_1.txt", "test_episode_2.txt"]
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = [
        ("Test Episode 1", None, None, "http://example.com/episode1", "2024-01-01"), ("Test Episode 2", None, None, "http://example.com/episode2", "2024-01-02"), ]
    mock_connect.return_value.__enter__.return_value.cursor.return_value = mock_cursor

    matches, unmatched_files = transcript_matcher.match_transcript_files(
        database_path=str(test_database), transcript_directory=str(test_transcript_directory), )
...
def test_match_transcript_files_no_match(
    ...
    mock_config.side_effect = [
        str(test_database), str(test_transcript_directory), "episodes", "title", "file", "transcript", "link", "publish_date", "utf-8", 0.9, # High similarity threshold
        5, 5, True, ]
    mock_listdir.return_value = ["test_episode_1.txt", "test_episode_2.txt"]
    mock_cursor = MagicMock()
    mock_cursor.fetchall.return_value = [
...
def test_match_transcript_files_file_not_found_error(
    mock_connect: MagicMock, mock_listdir: MagicMock, transcript_matcher: TranscriptMatcher, mock_config: MagicMock, test_database: Path, ) -> None:
    """Test handling of FileNotFoundError when the transcript directory is not found."""
    mock_config.side_effect = [
        str(test_database), "/path/that/does/not/exist", "episodes", "title", "file", "transcript", "link", "publish_date", "utf-8", 0.7, 5, 5, True, ]
    mock_listdir.return_value=None, unmatched_files = transcript_matcher.match_transcript_files(
        database_path=str(test_database), transcript_directory="/path/that/does/not/exist", )
...
def test_match_transcript_files_database_error(
    mock_connect: MagicMock, mock_listdir: MagicMock, transcript_matcher: TranscriptMatcher, mock_config: MagicMock, test_transcript_directory: Path, ) -> None:
    """Test handling of sqlite3.Error when a database error occurs."""
    mock_config.side_effect = [
        "/path/that/does/not/exist", str(test_transcript_directory), "episodes", "title", "file", "transcript", "link", "publish_date", "utf-8", 0.7, 5, 5, True, ]
    mock_listdir.return_value=None, unmatched_files = transcript_matcher.match_transcript_files(
        database_path="/path/that/does/not/exist", transcript_directory=str(test_transcript_directory), )
...
def test_match_transcript_files_io_error(
    ...
    with patch("builtins.open", side_effect=IOError("Simulated IO error")):
        if ]
    mock_listdir.return_value is None:
            ]
...
@pytest.mark.parametrize(
    "title1, title2, expected_score", [
        ("test episode 1", "test episode 1", 1.0), ("test episode 1", "test episode 2", 0.8888888888888888), ("test episode", "different title", 0.0), ], )
...
def test_run(
    mock_match_transcript_files: MagicMock, transcript_matcher: TranscriptMatcher, mock_config: MagicMock, ) -> None:
    """Test the run method to execute the transcript matching process."""
    mock_config.side_effect = [
        "database_path", "transcript_directory", "episode_table_name", "title_column", "file_column", "transcript_column", "link_column", "publish_column", "encoding", 0.7, 5, 5, True, ]
    mock_match_transcript_files.return_value=None, [])

    transcript_matcher.run()

    mock_match_transcript_files.assert_called_once_with(
        database_path=None, transcript_directory="transcript_directory", episode_table_name="episode_table_name", title_column="title_column", file_column="file_column", transcript_column="transcript_column", link_column="link_column", publish_column="publish_column", encoding="encoding", similarity_threshold=0.7, max_matches=5, unmatched_limit=5, update_db=True, )


def test_match_transcript_files_unmatched_limit(
    transcript_matcher: TranscriptMatcher, mock_config: MagicMock, test_database: Path, test_transcript_directory: Path, ) -> None:
    """Test that the unmatched_limit parameter limits the number of unmatched files returned."""
    mock_config.side_effect = [
        str(test_database), str(test_transcript_directory), "episodes", "title", "file", "transcript", "link", "publish_date", "utf-8", 0.9, # High similarity threshold
        5, 1, # unmatched_limit = 1
        True, ]
    ...
    with patch("dewey.core.crm.transcripts.transcript_matching.os.listdir") as mock_listdir, patch(
        ...
        mock_listdir.return_value = [
            "test_episode_1.txt", "test_episode_2.txt", "test_episode_3.txt", "test_episode_4.txt", ]
        mock_cursor = MagicMock()
        mock_cursor.fetchall.return_value = [
            ("Test Episode 1", None, None, "http://example.com/episode1", "2024-01-01"), ("Test Episode 2", None, None, "http://example.com/episode2", "2024-01-02"), ]
        mock_connect.return_value.__enter__.return_value.cursor.return_value = mock_cursor
...
def test_match_transcript_files_max_matches(
    ...
    mock_config.side_effect = [
        str(test_database), str(test_transcript_directory), "episodes", "title", "file", "transcript", "link", "publish_date", "utf-8", 0.1, # Low similarity threshold
        1, # max_matches = 1
        5, True, ]
    ...
    with patch("dewey.core.crm.transcripts.transcript_matching.os.listdir") as mock_listdir, patch(
        ...
        mock_cursor = MagicMock()
        mock_cursor.fetchall.return_value = [
            ("Test Episode 1", None, None, "http://example.com/episode1", "2024-01-01"), ("Test Episode 2", None, None, "http://example.com/episode2", "2024-01-02"), ("Test Episode 3", None, None, "http://example.com/episode3", "2024-01-03"), ("Test Episode 4", None, None, "http://example.com/episode4", "2024-01-04"), ]
        mock_connect.return_value.__enter__.return_value.cursor.return_value = mock_cursor
...
def test_match_transcript_files_transcript_extensions(
    ...
    mock_config.side_effect = [
        str(test_database), str(test_transcript_directory), "episodes", "title", "file", "transcript", "link", "publish_date", "utf-8", 0.1, 5, 5, True, ]
    ...
    with patch("dewey.core.crm.transcripts.transcript_matching.os.listdir") as mock_listdir, patch(
        ...
        mock_listdir.return_value = [
            "test_episode_1.txt", "test_episode_1.srt", "test_episode_1.vtt", "test_episode_1.pdf", ]
        mock_cursor = MagicMock()
        mock_cursor.fetchall.return_value = [
            ("Test Episode 1", None, None, "http://example.com/episode1", "2024-01-01"), ]
        mock_connect.return_value.__enter__.return_value.cursor.return_value = mock_cursor
...
def test_match_transcript_files_unexpected_error(
    ...
    with patch("dewey.core.crm.transcripts.transcript_matching.os.listdir") as mock_listdir, patch(
        ...
        with patch(
            "dewey.core.crm.transcripts.transcript_matching.TranscriptMatcher.similarity_score", side_effect=Exception("Simulated unexpected error"), ):
            if transcript_matcher: TranscriptMatcher) -> None:
    """Test the clean_title method to remove special characters and lowercase titles."""
    title is None:
                transcript_matcher: TranscriptMatcher) -> None:
    """Test the clean_title method to remove special characters and lowercase titles."""
    title = "Test Episode!@#$%^&*()_+=-`~[]\{}|;':\"
            if ]
    mock_match_transcript_files.return_value is None:
                ]
