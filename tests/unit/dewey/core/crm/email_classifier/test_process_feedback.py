...
def mock_openai():
    ...
    with patch(
        "src.dewey.core.crm.email_classifier.process_feedback.OpenAI"
    ) as MockOpenAI:
        yield MockOpenAI
...
def setup_test_environment(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.dewey.core.crm.email_classifier.process_feedback.ACTIVE_DATA_DIR",
        str(active_data_dir),
    )
    monkeypatch.setattr(
        "src.dewey.core.crm.email_classifier.process_feedback.DB_FILE",
        str(active_data_dir / "process_feedback.duckdb"),
    )
    monkeypatch.setattr(
        "src.dewey.core.crm.email_classifier.process_feedback.CLASSIFIER_DB",
        str(active_data_dir / "email_classifier.duckdb"),
    )
...
def test_generate_feedback_json_valid(mock_openai):
    ...
    mock_response.return_value.choices[0].message.content = (
        '{"msg_id": "test", "subject": "Test Subject", "assigned_priority": 3, "feedback_comments": "Good feedback", "suggested_priority": 2, "add_to_topics": null, "add_to_source": null}'
    )
...
def test_suggest_rule_changes_topic_suggestion():
    ...
    changes = suggest_rule_changes(feedback, {})
    assert any(
        c["type"] == "add_override_rule" and c["keyword"] == "important"
        for c in changes
    )
...
def test_suggest_rule_changes_source_suggestion():
    ...
    changes = suggest_rule_changes(feedback, {})
    assert any(
        c["type"] == "add_override_rule" and c["keyword"] == "example.com"
        for c in changes
    )
...
def test_update_preferences_add_existing_rule():
    ...
    assert updated["override_rules"][0]["keywords"] == ["test"]
    assert (
        updated["override_rules"][0]["min_priority"] == 1
...
def test_migration_from_legacy_files(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "os.path.exists", lambda x: x in [str(feedback_file), str(email_prefs_file)]
    )
    monkeypatch.setattr(
        "src.dewey.core.crm.email_classifier.process_feedback.DB_FILE",
        str(tmp_path / "process_feedback.duckdb"),
    )
    monkeypatch.setattr(
        "src.dewey.core.crm.email_classifier.process_feedback.CLASSIFIER_DB",
        str(tmp_path / "email_classifier.duckdb"),
    )
...
def test_main_flow_no_feedback(monkeypatch, mock_openai, capsys):
    ...
    mock_openai.return_value.chat.completions.create.return_value.choices[
        0
    ].message.content = '{"msg_id": "test", "subject": "Test Subject", "assigned_priority": 3, "feedback_comments": "Good feedback", "suggested_priority": 2, "add_to_topics": null, "add_to_source": null}'
    main()
...
def test_main_flow_add_new_feedback(monkeypatch, mock_openai, capsys):
    ...
    monkeypatch.setattr(
        "builtins.input",
        lambda prompt: "y" if "add new feedback" in prompt else "test feedback",
    )
    mock_openai.return_value.chat.completions.create.return_value.choices[
        0
    ].message.content = '{"msg_id": "test", "subject": "Test Subject", "assigned_priority": 3, "feedback_comments": "Good feedback", "suggested_priority": 2, "add_to_topics": null, "add_to_source": null}'
...
def test_main_flow_with_opportunities(monkeypatch, mock_openai, capsys):
    ...
    with patch(
        "src.dewey.core.crm.email_classifier.process_feedback.init_db"
    ) as mock_init_db:
        mock_conn = MagicMock()
...
@pytest.mark.parametrize(
    "assigned,suggested,expected_avg",
    [(3, 1, -2), (0, 4, 4), (2, 2, 0), (4, 0, -4), (1, 0, -1)],
)
...
def test_main_flow_interactive_feedback(monkeypatch, mock_openai, capsys):
    ...
    with patch(
        ...
        mock_conn.execute.return_value.fetchall.return_value = mock_opportunities
        monkeypatch.setattr(
            "builtins.input", lambda prompt: "test feedback"
        )  # Provide feedback
        mock_openai.return_value.chat.completions.create.return_value.choices[
            0
        ].message.content = '{"msg_id": "test", "subject": "Test Subject", "assigned_priority": 3, "feedback_comments": "Good feedback", "suggested_priority": 2, "add_to_topics": null, "add_to_source": null}'
        ...
        captured = capsys.readouterr()
        assert (
            "Type feedback, 't' to tag, 'i' for ingest, or 'q' to quit" in captured.out
        )
...
def test_main_flow_ingest_action(monkeypatch, mock_openai, capsys):
    ...
    with patch(
        ...
        mock_conn.execute.return_value.fetchall.return_value = mock_opportunities
        monkeypatch.setattr(
            "builtins.input", lambda prompt: "i" if "feedback" in prompt else "1"
        )  # Provide feedback
        mock_openai.return_value.chat.completions.create.return_value.choices[
            0
        ].message.content = '{"msg_id": "test", "subject": "Test Subject", "assigned_priority": 3, "feedback_comments": "Good feedback", "suggested_priority": 2, "add_to_topics": null, "add_to_source": null}'
...
def test_main_flow_show_more_emails(monkeypatch, mock_openai, capsys):
    ...
    with patch(
        ...
        mock_conn.execute.return_value.fetchall.return_value = mock_opportunities
        monkeypatch.setattr(
            "builtins.input", lambda prompt: "y" if "Show all" in prompt else "q"
        )  # Show all emails, then quit
        mock_openai.return_value.chat.completions.create.return_value.choices[
            0
        ].message.content = '{"msg_id": "test", "subject": "Test Subject", "assigned_priority": 3, "feedback_comments": "Good feedback", "suggested_priority": 2, "add_to_topics": null, "add_to_source": null}'
        main()
        captured = capsys.readouterr()
        assert (
            "Email 4: Subject 1" in captured.out
        )  # Verify that the 4th email is displayed
...
def test_main_flow_skip_show_more_emails(monkeypatch, mock_openai, capsys):
    ...
    with patch(
        ...
        mock_conn.execute.return_value.fetchall.return_value = mock_opportunities
        monkeypatch.setattr(
            "builtins.input", lambda prompt: "n" if "Show all" in prompt else "q"
        )  # Skip showing all emails, then quit
        mock_openai.return_value.chat.completions.create.return_value.choices[
            0
        ].message.content = '{"msg_id": "test", "subject": "Test Subject", "assigned_priority": 3, "feedback_comments": "Good feedback", "suggested_priority": 2, "add_to_topics": null, "add_to_source": null}'
        main()
        captured = capsys.readouterr()
        assert (
            "Email 4: Subject 1" not in captured.out
        )  # Verify that the 4th email is not displayed
...
def test_main_flow_quit_show_more_emails(monkeypatch, mock_openai, capsys):
    ...
    with patch(
        ...
        mock_conn.execute.return_value.fetchall.return_value = mock_opportunities
        monkeypatch.setattr(
            "builtins.input", lambda prompt: "q" if "Show all" in prompt else "q"
        )  # Quit while showing all emails
        mock_openai.return_value.chat.completions.create.return_value.choices[
            0
        ].message.content = '{"msg_id": "test", "subject": "Test Subject", "assigned_priority": 3, "feedback_comments": "Good feedback", "suggested_priority": 2, "add_to_topics": null, "add_to_source": null}'
        main()
        captured = capsys.readouterr()
        assert (
            "Exiting feedback session..." in captured.out
        )  # Verify that the session is exited
