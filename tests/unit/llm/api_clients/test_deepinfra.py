...
def test_initialization_without_api_key():
    ...
    with pytest.raises(LLMError, match="DeepInfra API key not found"):
        with patch("dewey.config.load_config") as mock_load_config:
            mock_load_config.return_value = {"llm": {"providers": {"deepinfra": {"api_key": None}}}}
            DeepInfraClient()
...
def test_rate_limit_error():
    ...
    with patch("openai.ChatCompletion") as mock_chat:
        mock_chat.completions.create.side_effect = Exception("429 Client Error: Too Many Requests")
        client = DeepInfraClient()
...
def test_missing_model_fallback():
    ...
    with patch("openai.ChatCompletion") as mock_chat:
        ...
        assert mock_chat.completions.create.call_args_list[1].kwargs["model"] == "fallback_model"
