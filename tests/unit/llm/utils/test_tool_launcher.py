...
@pytest.fixture
def tool_launcher():
    """Function tool_launcher."""
    return ToolLauncher()


@pytest.fixture
def mock_subprocess(mocker):
    """Function mock_subprocess."""
...
class TestToolLauncher:
    ...
    async def test_tool_launcher_verify_tools_missing_tool(self, tool_launcher, caplog):
        """Test that missing tools log a warning."""
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr(shutil, "which", lambda x: None)
            tool_launcher._verify_tools()
            assert (
                "Missing tools: visidata, gtop, wtfutil, frogmouth, calcure, harlequin"
                in caplog.text
            )
    ...
    async def test_launch_tool_missing_command(self, tool_launcher, mock_subprocess):
    ...
    async def test_launch_tool_process_tracking(self, tool_launcher, mock_subprocess):
    ...
    async def test_monitor_process_logs_errors(self, tool_launcher, mock_subprocess):
        ...
        assert (
            "test_tool stderr: error message" in tool_launcher.logger.handlers[0].buffer
        )
        assert "test_tool exited with code 1" in tool_launcher.logger.handlers[0].buffer


@pytest.mark.parametrize(
    "tool,args,expected_cmd",
    [
