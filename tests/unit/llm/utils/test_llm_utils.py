    ...
    return {
        ...
        "providers": {
            "deepinfra": {
                ...
                "fallback_models": ["google/gemini-2.0-pro"],
            }
        },
...
class TestLLMHandler:
    ...
    def test_generate_response_with_fallback(
        self, llm_config, mock_gemini_client, mock_deepinfra_client
    ):
        ...
        handler = LLMHandler(llm_config)
        response = handler.generate_response(
            "Test prompt", fallback_model="deepseek-ai/DeepSeek-R1-Distill-Qwen-32B"
        )
    ...
    def test_generate_response_both_fail(
        self, llm_config, mock_gemini_client, mock_deepinfra_client
    ):
    ...
    def test_json_extraction_non_strict(self, llm_config, mock_gemini_client):
        ...
        handler = LLMHandler(llm_config)
        response = handler.generate_response(
            "Test prompt", response_format={"type": "json_object"}, strict_json=False
        )
    ...
    def test_client_specific_parameters(
        self, llm_config, mock_gemini_client, mock_deepinfra_client
    ):
        """Test handling of client-specific parameters."""
        # Test Gemini parameters
        handler = LLMHandler(llm_config)
        handler.generate_response("Test prompt", temperature=0.5, max_tokens=100)
...
def test_validate_model_params_valid():
    """Test valid model parameters"""
    validate_model_params({"temperature": 0.5, "max_tokens": 500})


def test_validate_model_params_invalid_temperature():
    """Test invalid temperature raises ValueError"""
    with pytest.raises(ValueError, match="Temperature must be between 0 and 2"):
        validate_model_params({"temperature": 3})


def test_validate_model_params_invalid_max_tokens():
    """Test invalid max_tokens raises ValueError"""
    with pytest.raises(ValueError, match="max_tokens must be positive integer"):
        validate_model_params({"max_tokens": -10})
