...
def fix_imports(file_path, old_path, new_path):
    ...
    try:
        ...
        if "from .." in content or "from ." in content:
            ...
            print(
                f"WARNING: File {file_path} contains relative imports that may need manual inspection"
            )
        ...
        if old_rel_path.replace("tests/", "") in content:
            content = content.replace(
                old_rel_path.replace("tests/", ""), new_rel_path.replace("tests/", "")
            )
            modified = True
...
def copy_test_file(source, target, old_path, new_path):
    ...
    if not os.path.exists(target) or os.path.getmtime(source) > os.path.getmtime(
        target
    ):
...
def copy_root_files():
    ...
    for root_file in ROOT_FILES:
        ...
        if source.exists():
            ...
            if not target.exists() or os.path.getmtime(source) > os.path.getmtime(
                target
            ):
                shutil.copy2(source, target)
