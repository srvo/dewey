...
class TestDatabaseMonitor(unittest.TestCase):
    ...
    def test_check_sync_health(self):
        ...
        with patch('src.dewey.core.db.monitor.get_last_sync_time') as mock_sync:
            ...
            self.assertTrue(result["healthy"], f"Expected healthy sync but got {result}")
    ...
    def test_run_health_check(self):
        ...
        with patch('src.dewey.core.db.monitor.check_connection') as mock_conn:
            ...
            with patch('src.dewey.core.db.monitor.check_sync_health') as mock_sync:
                ...
                with patch('src.dewey.core.db.monitor.check_schema_consistency') as mock_schema:
                    mock_schema.return_value = {"consistent": True}
                    
                    with patch('src.dewey.core.db.monitor.check_database_size') as mock_size:
                        mock_size.return_value = {"file_size_bytes": 1024 * 1024}
                        
                        with patch('src.dewey.core.db.monitor.check_table_health') as mock_table:
                            mock_table.return_value = {"healthy": True}
                            
                            # Mock TABLES list
                            with patch('src.dewey.core.db.monitor.TABLES', ['test_table']):
                                # Run health check
                                ...
                                self.assertTrue(result["tables"]["test_table"]["healthy"])
