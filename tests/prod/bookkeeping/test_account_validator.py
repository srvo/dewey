...
class MockFileSystem(FileSystemInterface):
    ...
    def __init__(self, files: Dict[str, str] = None, existing_files: set = None) -> None:
...
class TestAccountValidator:
    ...
    @patch("json.load")
    def test_load_rules_invalid_json(self, mock_json_load: MagicMock, validator: AccountValidator) -> None:
    ...
    def test_validate_accounts_success(self, validator: AccountValidator) -> None:
        ...
        rules = validator.load_rules(Path("rules.json"))
        result = validator.validate_accounts(Path("journal.hledger"), rules, run_command=mock_run)
    ...
    def test_validate_accounts_missing_accounts(self, validator: AccountValidator) -> None:
    ...
    def test_validate_accounts_hledger_error(self, validator: AccountValidator) -> None:
        ...
        mock_run = MagicMock(side_effect=subprocess.CalledProcessError(1, "hledger", "Command failed"))
        
        rules = validator.load_rules(Path("rules.json"))
        
        with pytest.raises(subprocess.CalledProcessError):
            validator.validate_accounts(Path("journal.hledger"), rules, run_command=mock_run)
    ...
    @patch("sys.exit")
    def test_run_success(self, mock_exit: MagicMock, validator: AccountValidator) -> None:
        """Test successful execution of run method."""
        with patch.object(validator, 'load_rules') as mock_load:
            with patch.object(validator, 'validate_accounts', return_value=True) as mock_validate:
                # Configure mocks
    ...
    @patch("sys.argv", ["account_validator.py", "journal.hledger", "rules.json"])
    @patch("sys.exit")
    def test_run_validation_failure(self, mock_exit: MagicMock, validator: AccountValidator) -> None:
        """Test run method when validation fails."""
        with patch.object(validator, 'load_rules') as mock_load:
            with patch.object(validator, 'validate_accounts', return_value=False) as mock_validate:
                # Configure mocks
                mock_load.return_value = {"categories": ["Assets:Checking"]}
    ...
    @patch("sys.argv", ["account_validator.py", "journal.hledger", "rules.json"])
    @patch("sys.exit")
    def test_run_load_error(self, mock_exit: MagicMock, validator: AccountValidator) -> None:
        """Test run method when rules loading fails."""
        with patch.object(validator, 'load_rules', side_effect=Exception("Failed to load rules")):
            # Execute run
            validator.run()
    ...
    def test_run_journal_not_found(self, validator: AccountValidator, mock_sys_exit: MagicMock) -> None:
        """Test run method when journal file is not found."""
        mock_argv = ["account_validator.py", "nonexistent_journal.journal", "rules.json"]
        ...
        with patch("sys.argv", mock_argv):
            with patch("os.path.exists", lambda path: path != "nonexistent_journal.journal"):
                validator.run()
                
                # Check that sys.exit was called with the correct error code
                mock_sys_exit.assert_any_call(1)
                mock_logger.error.assert_any_call("Journal file not found: nonexistent_journal.journal")

    def test_run_rules_not_found(self, validator: AccountValidator, mock_sys_exit: MagicMock) -> None:
        """Test run method when rules file is not found."""
        mock_argv = ["account_validator.py", "journal.journal", "nonexistent_rules.json"]
        ...
        with patch("sys.argv", mock_argv):
            with patch("os.path.exists", lambda path: path != "nonexistent_rules.json"):
                ...
                # Check that sys.exit was called with the correct error code
                mock_sys_exit.assert_any_call(1)
                mock_logger.error.assert_any_call("Rules file not found: nonexistent_rules.json") 