...
from textual.widgets import (
    Header, Footer, Button, DataTable, Input, Static, 
    Label, Select, Checkbox, TextArea, LoadingIndicator
)
from textual.screen import Screen, ModalScreen

# Import database models and utilities
from dewey.core.db.models import (
    EmailFeedback, ClientCommunicationsIndex, ClientProfiles, Contacts
)
...
class FeedbackDatabase:
    ...
    def __init__(self, db_path: str = DB_FILE, md_conn: Optional[str] = MOTHERDUCK_DB):
    ...
    def get_email_threads(self, contact_email: Optional[str] = None) -> List[Dict[str, Any]]:
...
class ContactDetailModal(ModalScreen):
    ...
    async def load_communication_history(self) -> None:
        ...
        for thread in threads:
            ...
            date_str = received_time.strftime("%Y-%m-%d %H:%M") if received_time else "Unknown"
...
class FeedbackEditModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Container(id="feedback-modal"):
            ...
            with Vertical(id="feedback-details"):
                yield Label("Subject:")
                yield Input(value=self.feedback_item.subject, id="subject-input", disabled=True)
                
                yield Label("Original Priority:")
                yield Input(value=str(self.feedback_item.original_priority), id="orig-priority-input", disabled=True)
                ...
                yield Select(
                    ...
                    value=str(self.feedback_item.suggested_priority) if self.feedback_item.suggested_priority is not None else "",
                    id="suggested-priority-select"
                )
                
                yield Label("Feedback Comments:")
                yield TextArea(value=self.feedback_item.feedback_comments or "", id="feedback-comments")
                
                yield Label("Topics (comma separated):")
                yield Input(value=self.feedback_item.add_to_topics or "", id="topics-input")
                
                yield Checkbox("Flag for Follow-up", value=self.feedback_item.follow_up, id="follow-up-checkbox")
    ...
    def handle_save(self) -> None:
        ...
        self.feedback_item.assigned_priority = int(assigned_priority) if assigned_priority else 0
        
        suggested_priority = self.query_one("#suggested-priority-select", Select).value
        self.feedback_item.suggested_priority = int(suggested_priority) if suggested_priority else None
        
        self.feedback_item.feedback_comments = self.query_one("#feedback-comments", TextArea).value
        self.feedback_item.add_to_topics = self.query_one("#topics-input", Input).value
        self.feedback_item.follow_up = self.query_one("#follow-up-checkbox", Checkbox).value
...
class FeedbackManagerApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="main-container"):
            ...
            with Container(id="detail-container"):
                yield Static("Select a feedback item to view details", id="detail-header")
                with Vertical(id="feedback-details"):
    ...
    def setup_table(self) -> None:
        ...
        table.add_columns(
            "ID", "Subject", "Priority", "Suggested", 
            "Topics", "Follow Up", "Date", "Contact"
        )
        ...
        if self.search_query:
            ...
            self.filtered_items = [
                ...
                if (query in item.subject.lower() or
                    query in (item.feedback_comments or "").lower() or
                    query in (item.add_to_topics or "").lower() or
                    query in (item.contact_email or "").lower() or
                    query in (item.contact_name or "").lower())
            ]
        else:
    ...
    def update_table(self) -> None:
        ...
        for item in self.filtered_items:
            ...
            suggested = str(item.suggested_priority) if item.suggested_priority is not None else "-"
            ...
            table.add_row(
                short_id, subject, priority, suggested,
                topics, follow_up, date, contact
            )
    ...
    def update_detail_view(self) -> None:
        ...
        self.query_one("#detail-header", Static).update(f"Feedback Details")
    ...
    def clear_detail_view(self) -> None:
        ...
        self.query_one("#detail-header", Static).update("Select a feedback item to view details")
    ...
    async def action_view_contact(self) -> None:
        ...
        if not item.contact_email:
            self.notify("No contact email associated with this feedback", title="No Contact")
            return
            
        result = await self.push_screen(ContactDetailModal(
            item.contact_email,
            item.contact_name,
            item.contact_notes
        ), wait=True)
    ...
    async def action_delete_feedback(self) -> None:
        ...
        confirmation = f"Are you sure you want to delete the feedback for '{item.subject}'?"
