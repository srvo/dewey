...
from textual.widgets import (
    ...
    Select, Static, TextArea, LoadingIndicator
)
...
class Port5Screen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="main-container"):
            with Horizontal(id="search-container"):
                yield Input(placeholder="Enter ticker(s) separated by commas", id="ticker-input")
            ...
            with Horizontal(id="content-container"):
                with Vertical(id="companies-container"):
                    yield Static("Companies", id="companies-header", classes="section-header")
                    yield DataTable(id="companies-table")
                    
                with Vertical(id="details-container"):
                    yield Static("Analysis Results", id="details-header", classes="section-header")
    ...
    def update_analysis_details(self) -> None:
        ...
        if self.selected_company_index < 0 or self.selected_company_index >= len(self.analysis_results):
        ...
        if result.error:
            self.query_one("#company-name", Static).update(f"Company: {result.name} ({result.ticker})")
        ...
        self.query_one("#company-name", Static).update(f"Company: {result.name} ({result.ticker})")
        self.query_one("#risk-score", Static).update(f"Risk Score: {result.risk_score}/5")
        self.query_one("#confidence-score", Static).update(f"Confidence: {result.confidence_score:.2f if result.confidence_score else 'N/A'}")
        self.query_one("#recommendation", Static).update(f"Recommendation: {result.recommendation or 'N/A'}")
        
        themes_text = "Primary Themes: " + ", ".join(result.primary_themes) if result.primary_themes else "No themes identified"
        self.query_one("#themes", Static).update(themes_text)
        
        self.query_one("#summary-text", TextArea).value = result.summary or "No summary available"
    ...
    async def analyze_tickers(self, tickers: List[str]) -> None:
        ...
        self.query_one("#status-text", Static).update(f"Analyzing {len(tickers)} companies...")
        ...
        async for result in workflow.process_companies_by_tickers(tickers):
            ...
            risk_display = f"{analysis_result.risk_score}/5" if analysis_result.risk_score else "Error"
            ...
            # Update status
            self.query_one("#status-text", Static).update(f"Analyzed {len(self.analysis_results)}/{len(tickers)} companies")
        
        # Reset UI state
        self.is_analyzing = False
        self.query_one("#loading-indicator", LoadingIndicator).display = False
        self.query_one("#status-text", Static).update(f"Analysis complete: {len(self.analysis_results)} companies")
    ...
    async def load_top_companies(self, limit: int = 20) -> None:
        ...
        self.query_one("#status-text", Static).update(f"Loading top {limit} companies...")
        self.query_one("#loading-indicator", LoadingIndicator).display = True
        ...
        self.query_one("#status-text", Static).update(f"Loaded {len(companies)} companies")
        self.query_one("#loading-indicator", LoadingIndicator).display = False
        ...
        # Update status
        self.query_one("#status-text", Static).update(f"Ready to analyze {len(companies)} companies")
    ...
    def action_analyze(self) -> None:
        ...
        if not ticker_text:
            ...
            if self.companies:
                tickers = [company.get("ticker") for company in self.companies if company.get("ticker")]
