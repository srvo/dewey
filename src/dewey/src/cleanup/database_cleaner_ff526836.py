            ...
            for file in files:
                if self._is_stale_temp_file(os.path.join(root, file), days_old):
                    try:
                        ...
                        logger.warning(f"Cleanup skipped {file}: {e!s}")
    ...
    def cleanup_duplicate_checkpoints(self, session: Session) -> int:
        ...
        try:
            ...
            for (ticker,) in companies:
                ...
                to_delete = (
                    session.query(ResearchIteration.id)
                    .filter(
                        ResearchIteration.company_ticker == ticker,
                    )
                    .order_by(ResearchIteration.created_at.desc())
                    .offset(1)
                    .subquery()
                )
                ...
                if delete_count > self.SAFETY_CHECKS["max_delete_rows"]:
                    logger.error(
                        f"Skipping {ticker}: delete count {delete_count} exceeds safety limit",
                    )
                    continue
            ...
            session.rollback()
            logger.exception(f"Deduplication failed: {e!s}")
            raise
