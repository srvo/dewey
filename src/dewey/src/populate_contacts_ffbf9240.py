...
def extract_contacts_from_emails(db_path: str = "email_data.db") -> None:
    ...
    try:
        ...
        logger.exception(f"Error extracting contacts: {e!s}")
        conn.rollback()
        raise
    finally:
        conn.close()
...
def get_priority_from_type(contact_type: str, tags: str = "") -> int:
    ...
    if (
        ("prospect" in contact_type and "dropped" not in tags)
        or "adviser" in contact_type
        or ("supporter" in contact_type or "helper" in contact_type)
    ):
        return 4
    # Medium priority for media and institutional contacts
    if "media" in contact_type or "institutional" in contact_type:
        return 3
    return 1
...
def import_data_master_contacts(csv_path: str, db_path: str = "email_data.db") -> None:
    ...
    try:
        ...
        logger.exception(f"Error importing Data Master contacts: {e!s}")
        conn.rollback()
        raise
    finally:
        conn.close()
...
def import_subscribers(csv_path: str, db_path: str = "email_data.db") -> None:
    ...
    try:
        ...
        logger.exception(f"Error importing subscribers: {e!s}")
        conn.rollback()
        raise
    finally:
        conn.close()
...
def import_exported_contacts(csv_path: str, db_path: str = "email_data.db") -> None:
    ...
    try:
        ...
        logger.exception(f"Error importing exported contacts: {e!s}")
        conn.rollback()
        raise
    finally:
        conn.close()
...
def summarize_contacts(db_path: str = "email_data.db") -> None:
    ...
    try:
        ...
        for _domain, _count in top_domains:
            pass

        for _priority, _count in priority_dist:
            pass

    except Exception as e:
        logger.exception(f"Error summarizing contacts: {e!s}")
        raise
    finally:
        conn.close()
...
def import_from_sqlite_db(db_path: str, target_db: str = "email_data.db") -> None:
    ...
    try:
        ...
        for table in contact_tables:
            ...
            email_col = next(
                (col[1] for col in columns if "email" in col[1].lower()),
                None,
            )
            name_col = next(
                (col[1] for col in columns if "name" in col[1].lower()),
                None,
            )
        ...
        logger.exception(f"Error importing from {db_path}: {e!s}")
...
def main() -> None:
    ...
    try:
        ...
        logger.exception(f"Error in main: {e!s}")
        raise
