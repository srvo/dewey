...
class JsonResearchIntegration(BaseScript):
    ...
    def ensure_tables_exist(self, conn: duckdb.DuckDBPyConnection) -> None:
    ...
    def update_company_research_queries(self, conn: duckdb.DuckDBPyConnection, data: Dict[str, Any]) -> None:
        ...
        try:
            if not data or 'company' not in data or 'search_queries' not in data:
                self.logger.warning("No company or search queries data found in the JSON file")
                return
    ...
    def update_company_research_results(self, conn: duckdb.DuckDBPyConnection, data: Dict[str, Any]) -> None:
        ...
        try:
            if not data or 'company' not in data or 'research_results' not in data:
                self.logger.warning("No company or research results data found in the JSON file")
                return
    ...
    def process_directory(self, conn: duckdb.DuckDBPyConnection, directory_path: str) -> None:
        ...
        try:
            ...
            self.logger.info(f"Completed processing {len(research_files)} research JSON files")
        except Exception as e:
