...
# Refactored from: port_cli
# Date: 2025-03-16T16:19:11.849713
...
class PortCLI:
    ...
    @click.option("--backup-only", is_flag=True, help="Write only to Supabase")
    async def note(self, ticker, type, tags, backup_only) -> None:
        ...
        if content:
            with Progress() as progress:
                ...
                if backup_only:
                    ...
                    self.db.supabase.table("research_notes").insert(
                        {
                            "ticker": ticker,
                            "content": content,
                            "note_type": type,
    ...
    @cli.command()
    @click.option("--force", is_flag=True, help="Force sync all notes")
    async def sync(self, force) -> None:
    ...
    @click.option(
        "--source",
        type=click.Choice(["primary", "backup", "both"]),
        default="both",
    )
    async def review(self, ticker, source) -> None:
        ...
        if source in ["backup", "both"]:
            backup_notes = (
                self.db.supabase.table("research_notes")
                .select(
                    "*",
                )
                .eq("ticker", ticker)
