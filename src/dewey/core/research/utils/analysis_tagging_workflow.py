...
# Refactored from: analysis_tagging_workflow
# Date: 2025-03-16T16:19:11.966766
...
from ..loaders.duckdb_loader import CompanyData, DuckDBLoader
...
class AnalysisTaggingWorkflow:
    ...
    def __init__(self, engine: DeepSeekEngine, loader: Optional[DuckDBLoader] = None) -> None:
    ...
    async def process_companies_by_tick_range(
        ...
        for company in self.loader.load_companies_by_tick_range(min_tick, max_tick, limit):
            try:
                ...
                self.loader.save_research_results(
                    company.ticker,
                    {
...
if __name__ == "__main__":
    ...
    async def main() -> None:
        ...
        def print_result(result: Dict[str, Any]) -> None:
            """Prints the analysis result for a company."""
            if "error" in result:
        ...
        if args.tickers:
            ...
            async for result in workflow.process_companies_by_tick_range(
                min_tick, max_tick, args.limit, callback=print_result
            ):
                pass
