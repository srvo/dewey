...
# Refactored from: github_analyzer
# Date: 2025-03-16T16:19:08.429815
...
class RateLimitMonitor:
    ...
    def update_limit(self, api_name: str, remaining: int, limit: int) -> None:
        ...
        crossed_threshold = next(
            (
                ...
                if usage_percent >= t > self._limits[api_name]["last_threshold"]
            ),
            None,
        )
...
class LibraryMetrics(BaseModel):
    ...
    release_frequency: float | None  # releases per month
    code_velocity: float | None  # commits per week
    issue_resolution_rate: float | None  # percentage of closed issues
...
async def analyze_github_repo(client: APIClient, repo_url: str) -> dict:
    ...
    try:
        ...
        commit_data = await client.request(
            "GET",
            f"{api_url}/stats/commit_activity",
            "github",
        )

        # Fetch closed issues count
        issues_data = await client.request(
            "GET",
            f"{api_url}/issues?state=closed&per_page=1",
            "github",
        )
        ...
        recent_commits = (
            sum(week.get("total", 0) for week in commit_data[-4:]) if commit_data else 0
        )
        ...
        return {
            ...
            "commits_last_year": (
                sum(week.get("total", 0) for week in commit_data)
                if commit_data
                else None
            ),
        ...
        logger.exception(f"Error analyzing repo {repo_url}: {e}")
        return {}
...
async def analyze_documentation_quality(client: APIClient, api_url: str) -> float:
    ...
    try:
        ...
        logger.exception(f"Error analyzing documentation quality: {e}")
        return 0.0
...
async def analyze_test_coverage(client: APIClient, api_url: str) -> float:
    ...
    try:
        ...
        try:
            coverage_data = await client.request(
                "GET",
                f"{api_url}/contents/.coverage",
                "github",
            )
            if coverage_data:
        ...
        logger.exception(f"Error analyzing test coverage: {e}")
        return 0.0
...
async def main() -> None:
    ...
    try:
        with open("libraries_and_resources.md") as f:
            content = f.read()
    except Exception as e:
        logger.exception(f"Error reading markdown file: {e}")
        return
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        ...
        for lib in libraries:
            try:
                ...
                logger.exception(f"Error processing library {lib['name']}: {e}")
                continue
