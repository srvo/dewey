...
# Refactored from: yahoo_finance_engine
# Date: 2025-03-16T16:19:11.201741
...
class YahooFinanceEngine(BaseEngine):
    ...
    async def process(self) -> Dict[str, Any]:
        ...
        Not typically used for this engine as it's primarily accessed via
        fetch_history.
        """
    ...
    def fetch_history(
        ...
        for attempt in range(self.max_retries):
            try:
                self.logger.info(
                    f"Fetching data for {ticker} (attempt "
                ...
                if attempt < self.max_retries - 1:
                    ...
                    self.logger.warning(
                        f"Error fetching {ticker}, attempt "
                        f"{attempt + 1}/{self.max_retries}. "
                    ...
                    self.logger.error(
                        f"Failed to fetch {ticker} after {self.max_retries} "
                        f"attempts: {str(e)}"
                    )
                    return None
