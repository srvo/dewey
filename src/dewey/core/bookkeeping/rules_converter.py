...
def generate_rules_json(
    ...
    logger.info(f"Generated rules file: {output_file}")
    logger.info(f"Total patterns: {rules['stats']['total_patterns']}")
    logger.info(f"Patterns with examples: {rules['stats']['patterns_with_examples']}")
    logger.info(f"Unique categories: {len(rules['categories'])}")
...
def main() -> None:
    ...
    logger.info(f"Parsing rules file: {rules_file}")
    classifications = parse_rules_file(rules_file)

    logger.info(f"Analyzing transactions in: {journal_dir}")
    analyze_transactions(journal_dir, classifications)

    logger.info(f"Generating rules file: {output_file}")
