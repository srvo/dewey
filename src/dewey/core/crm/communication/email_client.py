...
from dewey.core.db.connection import get_connection, get_motherduck_connection
...
class EmailClient(BaseScript):
    ...
    def _setup_imap(self) -> None:
        ...
        try:
            ...
            imap_server = self.get_config_value("imap_server", os.environ.get("IMAP_SERVER", "imap.gmail.com"))
            imap_port = int(self.get_config_value("imap_port", os.environ.get("IMAP_PORT", "993")))
            ...
            if not username or not password:
                raise ValueError("Missing email credentials in configuration or environment variables")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # Fetch emails
            self.logger.info(f"Fetching emails from {folder} (limit: {limit}, since: {since_date})")
