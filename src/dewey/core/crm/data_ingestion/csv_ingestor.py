    ...
    def run(self) -> None:
        ...
        try:
            ...
            if not csv_file_path or not table_name:
                raise ValueError("CSV file path and table name must be specified in the configuration.")

            self.logger.info(f"Ingesting data from CSV file: {csv_file_path} into table: {table_name}")
