...
def fetch_emails(imap: imaplib.IMAP4_SSL, csv_file: str, days_back: int = 7, 
               max_emails: int = 100, batch_size: int = 10, historical: bool = False, 
    ...
    try:
        ...
        if historical:
            ...
            print(f"Found {len(message_numbers[0].split())} messages between {start} and {end}")
        else:
        ...
        print(f"Processing {min(len(message_numbers), max_emails)} emails in batches of {batch_size}")
        
        # Open CSV file for appending
        with open(csv_file, 'a', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'msg_id', 'thread_id', 'subject', 'from', 'to', 'date', 
                'raw_date', 'message_id', 'attachments', 'batch_id', 
                'import_timestamp'
            ]
            ...
            for i in range(0, min(len(message_numbers), max_emails), batch_size):
                ...
                print(f"Processing batch {i//batch_size + 1} of {len(batch)} messages")
                ...
                for num in batch:
                    try:
                        ...
                        if not msgid_match or not thrid_match:
                            print(f"Failed to extract Gmail IDs from response")
                            continue
                        ...
                        if total_processed % 10 == 0:
                            print(f"Progress: {total_processed}/{min(len(message_numbers), max_emails)} emails processed")
                            
                    except Exception as e:
                        print(f"Error processing message {num}: {str(e)}")
                        continue
                
                print(f"Completed batch {i//batch_size + 1}. Total processed: {total_processed}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--username", help="Gmail username (if not using GMAIL_USERNAME env var)")
    parser.add_argument("--password", help="Gmail password (if not using GMAIL_APP_PASSWORD env var)")
    parser.add_argument("--days_back", type=int, default=7, help="Number of days to look back for emails")
    parser.add_argument("--max_emails", type=int, default=1000, help="Maximum number of emails to import")
    parser.add_argument("--batch_size", type=int, default=10, help="Number of emails to process in each batch")
...
def main():
    ...
    if not username:
        print("Gmail username not provided via GMAIL_USERNAME environment variable or command line argument")
        sys.exit(1)
        
    if not password:
        print("Gmail password not provided via GMAIL_APP_PASSWORD environment variable or command line argument")
        sys.exit(1)
