...
try:
    ...
    class OpenAI:
        def __init__(self, **kwargs):
            ...
            self.chat = type('chat', (), {'completions': type('completions', (), {'create': lambda **kw: None})()})
...
def safe_print(message, is_background=False):
    ...
    if QUIET_MODE and (
        ...
        "Connected to" in message or 
        "Ensuring" in message or 
        "Loading" in message or
        "config" in message or 
        "Table counts" in message or
        "Checking" in message or
        "Database" in message or
        "Attaching" in message or
        "Initialize" in message
    ):
        return
...
def init_db(db_path: Optional[str] = None, use_memory_db: bool = False, use_local_db: bool = False) -> DatabaseConnection:
    ...
    try:
        ...
        if use_local_db and os.path.exists(CLASSIFIER_DB):
            ...
            try:
                ...
                safe_print(f"WARNING: Could not attach classifier database: {attach_error}")
    ...
    try:
        ...
        for _, row in df.iterrows():
            entry = {
                ...
                'original_priority': row['original_priority'] if 'original_priority' in row else 0,
                'assigned_priority': row['assigned_priority'] if 'assigned_priority' in row else 0,
                'suggested_priority': row['suggested_priority'] if 'suggested_priority' in row else 0,
                'feedback_comments': row['feedback_comments'] if 'feedback_comments' in row else '',
                'add_to_topics': json.loads(row['add_to_topics']) if 'add_to_topics' in row and row['add_to_topics'] else [],
...
def save_feedback(conn: DatabaseConnection, feedback_entries: List[Dict[str, Any]]) -> None:
    ...
    safe_print(f"Saving {len(feedback_entries)} feedback entries to {MOTHERDUCK_FEEDBACK_TABLE}")
        ...
        for entry in feedback_entries:
            ...
            try:
                ...
                if not existing_df.empty:
                    ...
                    safe_print(f"Inserting new entry with ID {unique_id} for msg_id: {msg_id}")
                    insert_sql = f"""
...
def load_preferences(conn: DatabaseConnection) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'id': row['id'] if 'id' in row else None,
            'override_rules': json.loads(row['override_rules']) if 'override_rules' in row and row['override_rules'] else [],
            ...
            'content_value_weight': row['content_value_weight'] if 'content_value_weight' in row else 1.0,
            'sender_history_weight': row['sender_history_weight'] if 'sender_history_weight' in row else 1.0,
            'priority_map': json.loads(row['priority_map']) if 'priority_map' in row and row['priority_map'] else {},
            'timestamp': row['timestamp'] if 'timestamp' in row else None
        }
    except Exception as e:
...
"""
    ...
    try:
        ...
        try:
            ...
            return {"error": error_msg, "feedback_text": feedback_text, "timestamp": datetime.now().isoformat()}
    except Exception as e:
        print(f"Error calling AI API: {e}")
        print(f"Check your DEEPINFRA_API_KEY and internet connection")
        return {"error": f"API error: {str(e)}", "feedback_text": feedback_text, "timestamp": datetime.now().isoformat()}
...
def process_feedback_worker(conn: DatabaseConnection):
    ...
    try:
        ...
        safe_print("Background worker started and ready to process feedback", is_background=True)
        ...
        while True:
            try:
                ...
                if item is None:
                    safe_print("Received shutdown signal, cleaning up", is_background=True)
                    feedback_queue.task_done()
                    break
                    
                # Unpack the item
                feedback_text, msg_id, subject, priority, suggested_priority_override = item
                
                safe_print(f"Processing feedback for email: {subject[:50]}...", is_background=True)
                
                try:
                    # Generate feedback JSON using OpenAI or DeepInfra
                    feedback_json = generate_feedback_json(feedback_text, msg_id, subject, priority)
                    
                    # Check and flush any captured output
                    check_and_flush_outputs()
                    
                    if not feedback_json:
                        safe_print(f"Error: Empty feedback JSON returned for {subject}", is_background=True)
                        feedback_queue.task_done()
                        continue
                        
                    # Update priority if specified
                    if suggested_priority_override is not None:
                        feedback_json["suggested_priority"] = suggested_priority_override
                    
                    safe_print(f"Saving feedback to MotherDuck database: {MOTHERDUCK_FEEDBACK_TABLE}", is_background=True)
                    
                    # Save to the database - wrap in try/except for better error reporting
                    try:
                        save_feedback(conn, [feedback_json])
                        safe_print(f"Successfully saved feedback to MotherDuck for: {subject[:50]}", is_background=True)
                    except Exception as save_error:
                        safe_print(f"ERROR SAVING TO MOTHERDUCK: {save_error}", is_background=True)
                        # Try to reconnect and save again
                        try:
                            safe_print("Attempting to reconnect to MotherDuck...", is_background=True)
                            new_conn = get_motherduck_connection(MOTHERDUCK_DB_NAME)
                            save_feedback(new_conn, [feedback_json])
                            safe_print("Successfully saved after reconnection", is_background=True)
                        except Exception as retry_error:
                            safe_print(f"RETRY FAILED: {retry_error}", is_background=True)
                except Exception as e:
                    safe_print(f"Error processing feedback in background: {e}", is_background=True)
                finally:
                ...
                safe_print(f"Unexpected error in background worker: {e}", is_background=True)
...
def generate_test_data():
    ...
    test_data = [
        ('test-msg-1', 'Monthly Investment Update', 2, 'financial-updates@example.com', 'Here is your monthly portfolio performance summary. Your investments have grown by 2.5% since last month...', 3),
        ('test-msg-2', 'Question about retirement planning', 1, 'client1@gmail.com', 'I\'ve been thinking about my retirement plan and had some questions about the projections we discussed...', 1),
        ('test-msg-3', 'Invitation: Industry Conference', 3, 'events@finance-conference.com', 'You\'re invited to speak at our upcoming Financial Advisors Summit. The event will be held on June 15th...', 2),
        ('test-msg-4', 'Urgent: Client account issue', 0, 'support@custodian.com', 'We\'ve detected an issue with one of your client\'s accounts. Please review and take action immediately...', 1),
        ('test-msg-5', 'Weekly Market Insights', 4, 'research@investment-firm.com', 'This week\'s market highlights: Tech stocks continued their rally, while energy sector faced headwinds...', 3),
        ('test-msg-6', 'Follow-up from our meeting', 1, 'client1@gmail.com', 'Thank you for meeting with me yesterday. I\'ve given some thought to your recommendations and would like to proceed...', 1),
        ('test-msg-7', 'Regulatory update: New compliance requirements', 2, 'compliance@regulator.gov', 'Important: New regulations affecting financial advisors will take effect on August 1st. You must update your policies...', 1),
        ('test-msg-8', 'Your subscription renewal', 3, 'billing@research-service.com', 'Your subscription to our premium research service will renew automatically in 14 days. If you wish to cancel...', 2),
        ('test-msg-9', 'Partnership opportunity', 1, 'bizdev@wealthtech.com', 'I represent a fintech company that has developed an innovative portfolio analysis tool. Would you be interested in a partnership?', 1),
        ('test-msg-10', 'Client referral', 0, 'colleague@advisor-network.com', 'I have a client who recently relocated to your area and needs local financial advice. Would you be open to a referral?', 1)
    ]
    
    return test_data

def main(auto_skip_threshold=None, show_all_skipped=False, use_local_db=False, fast_start=True, limit=DEFAULT_LIMIT, use_test_data=False):
    ...
    try:
        ...
        if not use_test_data:
            ...
            if fast_start:
                ...
                opportunities = load_emails_fast(conn, limit=limit)  # Fallback to fast method
        ...
        if opportunities:
            ...
            if conn is not None:
                    ...
                    if not existing_senders_df.empty:
                        for _, row in existing_senders_df.iterrows():
                            if 'from_address' in row:
                                ...
                                sender_feedback_counts[row['from_address']] = row['feedback_count']
                        safe_print(f"Found {len(senders_with_feedback)} senders with existing feedback")
                except Exception as e:
                    safe_print(f"Warning: Could not determine senders with existing feedback: {e}")
                        ...
                        if not existing_senders_df.empty:
                            for _, row in existing_senders_df.iterrows():
                                if 'from_address' in row:
                                    senders_with_feedback.add(row['from_address'])
                                    sender_feedback_counts[row['from_address']] = row['feedback_count']
                            safe_print(f"Found {len(senders_with_feedback)} senders with existing feedback (using emails table)")
                ...
                safe_print("Test mode: Assuming all senders are new (no feedback history)")
            ...
            if skipped_senders:
                safe_print(f"Automatically skipped {len(skipped_senders)} senders with {AUTO_SKIP_THRESHOLD}+ existing entries:")
                
                # Determine how many skipped senders to show
                senders_to_show = skipped_senders if show_all_skipped else skipped_senders[:5]
            ...
            for sender_idx, (from_addr, emails) in enumerate(sorted_senders, 1):
                ...
                safe_print(f"\n{'='*80}")
                safe_print(f"=== Sender {sender_idx}/{len(sorted_senders)}: {from_addr}{new_indicator} ===")
                safe_print(f"{'='*80}")
                ...
                if len(emails) > 3:
                    show_more = get_user_input(f"\n  This sender has {len(emails)} emails. Show all? (y/n/q): ", "n")
                    if show_more == 'q':
                        break
                    if show_more == 'y':
                ...
                for email in emails:
                    ...
                    while not feedback_processed:
                        ...
                        user_input = get_user_input("\nType feedback, 't' to tag, 'i' for ingest, 'n' for next email, 's' for next sender, or 'q' to quit: ", "n")
                        
                        if user_input in ('q', 'quit'):
                            safe_print("\nExiting feedback session...")
                            # Wait for background processing to complete
                            safe_print("Waiting for background processing to complete...")
                            feedback_queue.join()
                        ...
                        if user_input == 't':
                            ...
                            safe_print("  1) Form submission (questions, contact requests)")
                        ...
                        suggested_priority = get_user_input("Suggested priority (0-4, blank to keep current): ", "")
                        
                        # Process the suggested priority
                        suggested_priority_override = None
                        if suggested_priority.isdigit():
                            suggested_priority_override = max(0, min(int(suggested_priority), 4))
                        
                        # Instead of processing synchronously, add to the background queue
                        feedback_queue.put((feedback_text, msg_id, subject, priority, suggested_priority_override))
                        ...
                        safe_print("You can continue with the next email while we process your feedback.")
...
def load_emails_fast(conn: DatabaseConnection, limit: int = 20) -> list:
    ...
    try:
        ...
        if not tables_df.empty:
            ...
            email_tables = tables_df[tables_df['table_name'].isin(['emails', 'email_analyses'])]
            if not email_tables.empty:
                for _, row in email_tables.iterrows():
                    safe_print(f"Found table: {row['table_schema']}.{row['table_name']}")
                    
                    # Check row count
                    try:
                        count_df = conn.execute(f"SELECT COUNT(*) FROM {row['table_name']}")
                        if not count_df.empty:
                            count = count_df.iloc[0, 0]
                            safe_print(f"Table {row['table_name']} has {count} rows")
                    except Exception as count_err:
                        safe_print(f"Error counting rows in {row['table_name']}: {count_err}")
        ...
        try:
            ...
            if not results.empty:
                safe_print(f"Found {len(results)} emails in {MOTHERDUCK_EMAILS_TABLE} table")
        ...
        try:
            ...
            if not results.empty:
                safe_print(f"Found {len(results)} emails in {MOTHERDUCK_EMAIL_ANALYSES_TABLE} table")
        ...
        return [
            ('test-msg-1', 'Test Email Subject 1', 2, 'test@example.com', 'This is a test email snippet for classification...', 1),
            ('test-msg-2', 'Please review proposal', 1, 'client@company.com', 'I wanted to follow up on the proposal we discussed last week...', 1),
            ('test-msg-3', 'Inquiry about services', 3, 'prospect@gmail.com', 'Hello, I found your website and am interested in learning more about your services...', 1),
            ('test-msg-4', 'Urgent: Server Down', 0, 'alerts@monitoring.com', 'ALERT: The primary database server is not responding. Please check immediately...', 1),
            ('test-msg-5', 'Newsletter Subscription', 4, 'marketing@newsletter.com', 'Thank you for subscribing to our newsletter. Here are this week\'s top stories...', 1),
        ]
    except Exception as e:
...
def test_mode():
    ...
    for sender_idx, (from_addr, emails) in enumerate(sorted_senders, 1):
        safe_print(f"\n{'='*80}")
        safe_print(f"=== Sender {sender_idx}/{len(sorted_senders)}: {from_addr} (TEST) ===")
        safe_print(f"{'='*80}")
        ...
        for email in emails:
            ...
            while not feedback_processed:
                ...
                processing_thread = threading.Thread(
                    ...
                    args=(feedback_text, msg_id, subject, priority, suggested_priority if suggested_priority else None),
                    daemon=True
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Interactive tool for email feedback processing")
    parser.add_argument(
        "--auto-skip-threshold", 
        type=int, 
    ...
    if args.quick:
        try:
            ...
            print("If you want to use test data for development, run with the --test flag.")
            sys.exit(1)
    ...
    try:
        ...
        if args.disable_auto_skip:
            main(auto_skip_threshold=9999, show_all_skipped=args.show_skipped, use_local_db=args.use_local_db, fast_start=not args.slow, limit=args.limit, use_test_data=args.test)
        else:
            main(auto_skip_threshold=args.auto_skip_threshold, show_all_skipped=args.show_skipped, use_local_db=args.use_local_db, fast_start=not args.slow, limit=args.limit, use_test_data=args.test)
    except Exception as e:
        print(f"\n========== ERROR: {str(e)} ==========")
