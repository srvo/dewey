...
class OAuthClientFilter(logging.Filter):
    """Filter out specific OAuth client messages from logs."""
...
def get_all_label_ids(service: Any) -> Dict[str, str]:
...
def prioritize_with_deepinfra(
    email_content: str, api_key: str
) -> Tuple[int, float]:
...
def label_email(
    service: Any, message_id: str, priority: int, label_cache: Dict[str, str]
) -> bool:
...
def fetch_emails(service: Any, max_results: int = 10) -> List[Dict]:
...
def log_edge_case(case_data: Dict[str, Any]):
    ...
    Args:
    ----
        case_data (Dict[str, Any]): Dictionary containing edge case details including:
            - type: Edge case type
            - subject: Email subject
...
def summarize_results(processed_emails: List[Dict[str, Any]]):
    ...
    Args:
    ----
        processed_emails (List[Dict[str, Any]]): List of processed email dictionaries containing:
            - subject: Email subject
    ...
    sender_groups: Dict[str, Any] = {}  # Track priorities by sender
...
def load_preferences() -> Dict[str, Any]:
    ...
    Returns:
    -------
        Dict[str, Any]: Dictionary containing prioritization preferences
