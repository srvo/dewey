...
def get_unprocessed_emails(session: Session) -> List[RawEmail]:
    """Fetches all unprocessed emails from the database.

    Args:
        session: SQLAlchemy session object.

    Returns:
        A list of RawEmail objects that have not been processed.
    """
    return (
...
def process_email(email: RawEmail, session: Session) -> None:
    """Processes a single email and marks it as processed.
...
def analyze_emails() -> None:
    ...
    try:
        ...
        raw_emails: List[RawEmail] = get_unprocessed_emails(session)
