...
def prepare_email_content(subject: str, snippet: str) -> str:
    r"""Prepare and sanitize email content for prioritization analysis.
...
def get_all_label_ids(service) -> dict[str, str]:
    ...
    try:
        ...
        logging.exception(f"Error retrieving labels: {e!s}")
        return {}
...
def prioritize_with_deepinfra(email_content: str, api_key: str) -> tuple[int, float]:
    r"""Prioritize email using DeepInfra API with retry logic.
    ...
    try:
        ...
        try:
            ...
            if not (0 <= priority <= 5 and 0 <= confidence <= 1):
                msg = "Invalid priority or confidence values"
                raise ValueError(msg)

            return priority, confidence

        except (json.JSONDecodeError, KeyError, ValueError) as e:
            logging.exception(f"Error parsing API response: {e!s}")
            return None, 0.0

    except Exception as e:
        logging.exception(f"Error calling DeepInfra API: {e!s}")
        raise
    ...
    try:
        ...
        if not label_id:
            try:
                ...
                logging.exception(f"Error creating label '{label_name}': {e!s}")
                return False
        ...
        logging.exception(f"Error labeling email {message_id}: {e!s}")
        return False
...
def fetch_emails(service, max_results: int = 10) -> list[dict]:
    ...
    """
    query = "(label:inbox OR label:sent) -label:Priority/Marketing -label:Priority/Very Low -label:Priority/Low \
    ...
    try:
            ...
            for message in results.get("messages", []):
                ...
                try:
                    ...
                    if "failedPrecondition" in str(e):
                        ...
                        logging.exception(
                            f"Error fetching email {message['id']}: {e!s}",
                        )
                    continue
        ...
        logging.exception(f"Error fetching email list: {e!s}")
        return []
...
def authenticate_gmail() -> any:
    ...
    try:
        ...
        logging.exception(f"Unexpected error during authentication: {e!s}")
        sys.exit(1)


def log_edge_case(case_data: dict) -> None:
    ...
    try:
        ...
        logging.exception(f"Failed to log edge case: {e!s}")


def summarize_results(processed_emails: list[dict]) -> None:
...
def load_preferences() -> dict:
    ...
    """
    try:
        with open("scripts/email_preferences.json") as f:
            return json.load(f)
    except Exception as e:
        logging.warning(f"Could not load preferences: {e!s}. Using defaults.")
        return {}
...
def check_contact_priority(from_email: str) -> tuple[int, str]:
    ...
    try:
        ...
        cursor.execute(
            "SELECT email, name, priority FROM contacts WHERE email = ?",
            (clean_email,),
        )
        ...
        logging.exception(f"Error checking contacts database: {e!s}")
...
def adjust_priority_based_on_preferences(
    ...
    r"""Adjust email priority based on user preferences and contact information.
    ...
    for source in preferences.get("low_priority_sources", []):
        if any(keyword.lower() in content_lower for keyword in source["keywords"]):
            ...
            if initial_priority > source["max_priority"]:
                return source["max_priority"], f"Adjusted down: {source['reason']}"

    # Apply newsletter defaults
    for config in preferences.get("newsletter_defaults", {}).values():
    ...
    try:
        ...
        for message in tqdm(messages, desc="Processing Emails"):
            ...
            try:
                ...
                failed += 1
                logging.exception(
                    f"Error processing email: {message['subject']} - {e!s}",
                )
                log_edge_case(
                    {
        ...
        logging.critical(f"An unexpected error occurred: {e!s}")
        log_edge_case(
            {
                "type": "critical_error",
                "error": str(e),
