...
class UnifiedEmailProcessor(BaseScript):
    ...
    def __init__(self):
        ...
        # Configuration
        self.sync_interval = self.get_config_value("sync_interval_seconds", 300)  # 5 minutes default
        self.max_results_per_sync = self.get_config_value("max_results_per_sync", 1000)
        
        # Email signature patterns from config
        self.signature_patterns = self.get_config_value("regex_patterns.contact_info", {
        ...
        try:
            ...
            self.logger.warning("EmailEnrichment module not found, continuing without it")
            self.enrichment = None
        
        # Setup database tables once during initialization
        try:
            self._setup_database_tables()
            self.logger.info("Database tables set up successfully during initialization")
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"❌ Error processing unprocessed emails: {e}", exc_info=True)
    ...
    def setup_gmail_client(self):
        ...
        try:
            ...
            credentials_dir = self.get_config_value("paths.credentials_dir", "config/credentials")
            credentials_path = self.get_path(os.path.join(credentials_dir, "credentials.json"))
            token_path = self.get_path(os.path.join(credentials_dir, "gmail_token.json"))
            
            self.logger.info(f"Initializing Gmail client with credentials from {credentials_path}")
    ...
    def sync_new_emails(self):
        ...
        self.gmail_sync.run(
            initial=False,
            max_results=self.max_results_per_sync
        )
    ...
    def process_unprocessed_emails(self):
        ...
        batch_size = self.get_config_value("batch_size", 100)  # Larger batch size for faster processing
        max_emails = self.get_config_value("max_emails", 1000)
        
        # Add a small delay between write operations to reduce concurrent writes
        write_delay = self.get_config_value("write_delay", 0.1)  # 100ms delay between writes
        ...
        try:
            total_count_query = """
                ...
                LIMIT ?
            """
            total_count_result = db_manager.execute_query(total_count_query, [max_emails])
            ...
            self.logger.info(f"📨 Found {total_count} emails to process (limiting to {max_emails})")
        except Exception as e:
        ...
        while processed_count < max_emails:
            ...
            try:
                ...
                remaining_time = (total_count - processed_count) / avg_rate if total_count and avg_rate > 0 else "unknown"
                ...
                try:
                    ...
                    if hasattr(self, 'had_write_conflicts') and self.had_write_conflicts:
                ...
                if write_conflicts and write_delay < 0.5:  # Cap at 500ms
                    ...
                    write_delay = min(write_delay * 1.5, 0.5)  # Increase by 50% up to 500ms
                    self.logger.info(f"Write conflicts detected, increasing delay from {old_delay:.2f}s to {write_delay:.2f}s")
        ...
        if 'email_analyses' not in existing_tables:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.warning(f"Could not create indexes on email_analyses: {e}")
            ...
            try:
                ...
                """)
                existing_columns = [row[0].lower() for row in columns_result] if columns_result else []
                ...
                for col_name, col_type in required_columns.items():
                    if col_name.lower() not in existing_columns:
                        self.logger.info(f"➕ Adding missing column to email_analyses: {col_name}")
                        try:
                            default_value = ""
                            if col_type == 'BOOLEAN':
                            ...
                            self.logger.warning(f"Could not add column {col_name} to email_analyses: {e}")
        ...
        if 'contacts' in existing_tables:
            try:
                ...
                for col_name, col_type in expected_columns.items():
                    if col_name.lower() not in existing_columns:
                        self.logger.info(f"➕ Adding missing column to contacts: {col_name}")
                        try:
                            default_value = ""
                            if col_type == 'BOOLEAN':
                                default_value = "DEFAULT FALSE"
                            ...
                            self.logger.warning(f"Could not add column {col_name} to contacts: {e}")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return [row[1].lower() for row in results if row[1] not in ('sqlite_master', 'sqlite_temp_master')]
                except Exception as e:
    ...
    def _process_single_email(self, email_id):
        ...
        try:
            ...
            if result:
                self.logger.debug(f"Email {email_id} already in email_analyses, checking if processing is complete")
                ...
                if status_result:
                    self.logger.debug(f"Email {email_id} already fully processed, skipping")
                    return True
            ...
            msg_id, thread_id, from_address, subject, internal_date, snippet = email_data[0]
            ...
            if hasattr(self, 'enrichment') and self.enrichment:
                try:
                    if hasattr(self.enrichment, 'enrich_email'):
                        self.logger.info(f"Performing additional enrichment for email {email_id}")
                        success = self.enrichment.enrich_email(email_id)
                        if success:
                            self.logger.info(f"Additional enrichment completed for email {email_id}")
                        else:
                            self.logger.warning(f"Additional enrichment failed for email {email_id}")
                    else:
                        self.logger.warning("EmailEnrichment class exists but missing enrich_email method")
                except Exception as e:
                    self.logger.error(f"Error during email enrichment: {e}", exc_info=True)
            ...
            self.logger.error(f"❌ Error processing single email {email_id}: {e}", exc_info=True)
            return False
    ...
    def _get_column_names(self, table_name: str) -> List[str]:
        ...
        try:
            ...
            if not column_names:
                self.logger.warning(f"Could not get column names, using fallback approach")
                if table_name == 'email_analyses':
                    column_names = ["msg_id", "thread_id", "subject", "from_address", 
                                   "status", "metadata", "priority", "priority_score", 
                                   "analysis_date", "snippet", "internal_date"]
                elif table_name == 'contacts':
                    column_names = ["email", "first_name", "last_name", "full_name", 
                                   "company", "job_title", "phone", "country", "source",
                                   "domain", "last_interaction_date", "metadata", "is_client"]
    ...
    def _store_email_analysis(self, msg_id, thread_id, subject, from_address, internal_date, snippet, contact):
        ...
        try:
            ...
            exists = db_manager.execute_query(
                "SELECT 1 FROM email_analyses WHERE msg_id = ?", 
                [msg_id]
            )
            ...
            if internal_date:
                try:
                    ...
                    if isinstance(internal_date, (int, float)):
                        ...
                        internal_date_str = datetime.fromtimestamp(internal_date_int / 1000 
                                                                    if internal_date_int > 1e10 
                                                                    else internal_date_int).isoformat()
                    # If it's a string, convert to int if possible
                    elif isinstance(internal_date, str):
                        internal_date_str = internal_date
                        try:
                            # Try to parse the string as a datetime
                            dt = datetime.fromisoformat(internal_date.replace('Z', '+00:00'))
            ...
            if exists:
                ...
                try:
                    ...
                    self._update_timestamp_fields(msg_id, now_str, now_int, column_names)
                    ...
                    self._update_internal_date(msg_id, internal_date_int, internal_date_str, column_names)
                    
                except Exception as e:
                    self.logger.warning(f"Error updating record, trying simpler update: {e}")
                ...
                try:
                    ...
                    essential_fields = [
                        "msg_id", "thread_id", "subject", "from_address", 
                        "priority", "priority_score", "metadata"
                    ]
                    
                    # Exclude any fields not present in the schema
                    fields_to_insert = [field for field in essential_fields if field.lower() in column_names]
            ...
            self.logger.error(f"Error storing email analysis for {msg_id}: {e}", exc_info=True)

    def _update_timestamp_fields(self, msg_id, timestamp_str, timestamp_int, column_names):
        ...
        if "analysis_date" in column_names:
            try:
                ...
                if not type_result:
                    self.logger.warning(f"Could not determine column type for analysis_date, skipping update")
                    return
                ...
                if any(t in col_type for t in ["TIMESTAMP", "DATETIME", "DATE"]):
                    db_manager.execute_query("""
                        ...
                        SET analysis_date = ?
                        WHERE msg_id = ?
                    """, [timestamp_str, msg_id], for_write=True)
                    self.logger.debug(f"Updated analysis_date with string value: {timestamp_str}")
                ...
                # For numeric types, use integer format
                    ...
                    db_manager.execute_query("""
                        ...
                        SET analysis_date = ?
                        WHERE msg_id = ?
                    """, [timestamp_int, msg_id], for_write=True)
                    self.logger.debug(f"Updated analysis_date with integer value: {timestamp_int}")
                    
                # For unknown types, skip update to avoid errors
                else:
                    self.logger.warning(f"Unsupported column type for analysis_date: {col_type}, skipping update")
                    
            except Exception as e:
                self.logger.warning(f"Could not update analysis_date: {e}")

    def _update_internal_date(self, msg_id, internal_date_int, internal_date_str, column_names):
        ...
        try:
            ...
            if not type_result:
                self.logger.warning(f"Could not determine column type for internal_date, skipping update")
                return
            ...
            if any(t in col_type for t in ["INT", "BIGINT", "INTEGER"]):
                if internal_date_int is not None:
                    db_manager.execute_query("""
                        ...
                        SET internal_date = ?
                        WHERE msg_id = ?
                    """, [internal_date_int, msg_id], for_write=True)
                    self.logger.debug(f"Updated internal_date with integer value: {internal_date_int}")
                else:
                    self.logger.warning(f"No integer value available for internal_date, skipping update")
            ...
            # For timestamp/date types, use string format
                ...
                if internal_date_str:
                    db_manager.execute_query("""
                        ...
                        SET internal_date = ?
                        WHERE msg_id = ?
                    """, [internal_date_str, msg_id], for_write=True)
                    self.logger.debug(f"Updated internal_date with string value: {internal_date_str}")
                else:
                    self.logger.warning(f"No string value available for internal_date, skipping update")
                
            # For unknown types, skip update to avoid errors
            else:
                self.logger.warning(f"Unsupported column type for internal_date: {col_type}, skipping update")
    ...
    def _enrich_contact_from_email(self, email_id: str, contact_info: Dict[str, Any]):
        ...
        if not contact_info.get('email'):
            self.logger.warning(f"No email found for contact in message {email_id}, using sender")
        ...
        for field in ['first_name', 'last_name', 'full_name', 'phone', 'company', 'title']:
        ...
        try:
            ...
            if client_match:
                simple_contact['confidence_score'] = min(1.0, simple_contact['confidence_score'] + 0.3)
        ...
        try:
            ...
            if contact_exists:
                ...
                try:
                    db_manager.execute_query("""
                        ...
                        SET last_updated = ?,
                            ...
                            confidence_score = ?
                        WHERE email = ?
                    """, [now_str, simple_contact['confidence_score'], simple_contact['email']], for_write=True)
                    self.logger.debug(f"Updated existing contact: {simple_contact['email']}")
                except Exception as e:
                    ...
                    try:
                        db_manager.execute_query("""
                            ...
                            SET last_updated = ?
                            WHERE email = ?
                        """, [now_str, simple_contact['email']], for_write=True)
                    except Exception as e2:
                        self.logger.error(f"Both update attempts failed for contact {simple_contact['email']}: {e2}")
            else:
                ...
                try:
                    ...
                    self.logger.debug(f"Inserted new contact: {simple_contact['email']}")
                    
                    # Try to update with additional fields if they exist
                    if simple_contact.get('first_name') or simple_contact.get('last_name'):
                        try:
                            update_query = """
                                UPDATE contacts 
                                SET first_name = ?,
                            ...
                            self.logger.warning(f"Could not update additional contact info: {e}")
                except Exception as e:
                    ...
                    try:
                        ...
                        if not exists_check or exists_check[0][0] == 0:
                            ...
                            """, [simple_contact['email'], now_str, now_str], for_write=True)
                    except Exception as e2:
                        self.logger.error(f"All insert attempts failed for {simple_contact['email']}: {e2}")
        except Exception as e:
            self.logger.error(f"Error enriching contact from email: {e}", exc_info=True)
    
    def _calculate_priority_score(self, contact_info: Dict[str, Any], body: str) -> float:
        """Calculate priority score based on contact and email content."""
        # Get priority weights from config
        priority_weights = self.get_config_value("crm.enrichment.priority_weights", {
            'is_client': 0.5,
            'title_match': 0.2,
            'company_match': 0.1,
            'urgent_keywords': 0.2,
        ...
        if contact_info.get('title'):
            if any(word in contact_info['title'].lower() for word in ['ceo', 'founder', 'director', 'vp', 'president']):
        ...
        opportunity_keywords = ['opportunity', 'proposal', 'contract', 'deal', 'partnership']
    ...
    def __signal_handler(self, sig, frame):
        ...
        if not hasattr(self, '_interrupted'):
            ...
            self.logger.info("⏹️ Received interrupt signal, finishing current email before shutdown...")
        else:
...
def main():
    ...
    try:
        ...
        try:
            if 'processor' in locals() and hasattr(processor, 'gmail_sync') and processor.gmail_sync:
