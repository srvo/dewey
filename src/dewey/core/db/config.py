...
def get_db_config() -> Dict:
    ...
    return {
        ...
        'backup_retention_days': int(os.getenv('DEWEY_BACKUP_RETENTION_DAYS', str(BACKUP_RETENTION_DAYS)))
    }
...
def validate_config() -> bool:
    ...
    if not IS_TEST_MODE:
        try:
            ...
            logger.warning(f"Could not create directories: {e}. This is expected in test environments.")
...
def setup_logging(log_level: str = 'INFO',
    ...
    if log_file and not IS_TEST_MODE:
        try:
            ...
            logger.warning(f"Could not create log directory: {e}. Logging to console only.")
