...
from dewey.core.db.connection import get_local_connection, get_motherduck_connection, DatabaseConnection
...
class DuckDBSync(BaseScript):
    ...
    def __init__(
        ...
        motherduck_db: str = "dewey",
        motherduck_token: Optional[str] = None,
        sync_interval: int = 6 * 60 * 60,  # 6 hours by default
        max_sync_age: int = 7 * 24 * 60 * 60,  # 7 days by default
        auto_sync: bool = True
    ) -> None:
        ...
        self.local_db_path = local_db_path or str(Path.home() / "dewey" / "dewey.duckdb")
        ...
        try:
            ...
            self.motherduck_conn = get_motherduck_connection(self.motherduck_db, motherduck_token)
    ...
    def list_tables(self, connection: DatabaseConnection) -> List[str]:
        ...
        try:
            ...
            filtered_tables = [t for t in tables if not t.startswith('sqlite_') and 
    ...
    def _record_sync_end(self, sync_id: int, status: str, tables_synced: int, records_synced: int, error_message: str = "") -> None:
        ...
        with self._sync_lock:
            ...
            try:
                ...
                for table_name in md_tables:
                    ...
                    if (table_name.startswith('sqlite_') or 
                        table_name.startswith('dewey_sync_')):
                        continue
                ...
                self.logger.info(f"Sync from MotherDuck to local completed. {tables_synced}/{len(md_tables)} tables synced.")
                return success
            except Exception as e:
    ...
    def sync_modified_to_motherduck(self) -> bool:
        ...
        with self._sync_lock:
            ...
            try:
                ...
                self.logger.info(f"Sync from local to MotherDuck completed. {tables_synced}/{len(modified_tables)} tables synced.")
                return success
            except Exception as e:
    ...
    def run(self) -> None:
        """Run method required by BaseScript interface."""
        self.sync_all()
...
def get_duckdb_sync(
    ...
    if _duckdb_sync_instance is None:
        _duckdb_sync_instance = DuckDBSync(
            ...
            motherduck_db=motherduck_db or "dewey",
            motherduck_token=motherduck_token,
            sync_interval=sync_interval or 6 * 60 * 60,  # 6 hours by default
            max_sync_age=max_sync_age or 7 * 24 * 60 * 60,  # 7 days by default
            auto_sync=auto_sync
        )
