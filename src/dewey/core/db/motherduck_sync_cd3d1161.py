...
def sync_database(db_file: Path, md_con: DuckDBBackend) -> Dict[str, Any]:
    """Sync all tables from a single DuckDB database to MotherDuck.
    ...
    Returns:
        A dictionary containing the sync results for the database.
    """
    result: Dict[str, Any] = {"tables_synced": [], "errors": []}
...
def sync_to_motherduck(db_directory: str, token: str) -> Dict[str, Any]:
    ...
    Args:
        db_directory: The directory containing the DuckDB files.
        token: The MotherDuck token.

    Returns:
        A dictionary containing the sync results.
    """
    result: Dict[str, Any] = {
        "success": False,
        "databases_processed": [],
    ...
    try:
        ...
        for db_file in db_path.glob("*.duckdb"):
            db_sync_result = sync_database(db_file, md_con)
            result["databases_processed"].append(db_file.name)
            result["tables_synced"].extend(db_sync_result["tables_synced"])
            result["errors"].extend(db_sync_result["errors"])
...
if __name__ == "__main__":
    load_dotenv()
    parser = argparse.ArgumentParser(description="Sync DuckDB databases to MotherDuck")
    parser.add_argument(
        "directory",
        nargs="?",
    ...
    if result["success"]:
        logger.info(f"Sync successful. Tables synced: {len(result['tables_synced'])}")
    else:
