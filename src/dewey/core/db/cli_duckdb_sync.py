...
from dewey.core.db.sync import DuckDBSync, get_duckdb_sync
...
class SyncDuckDBScript(BaseScript):
    ...
    def _parse_args(self) -> argparse.Namespace:
        ...
        parser.add_argument(
            '--tables',
            help='Specific tables to sync (comma-separated)',
            type=str
        )
        
        parser.add_argument(
            '--exclude',
            help='Tables to exclude from sync (comma-separated)',
            type=str
        )
    ...
    def run(self) -> None:
        ...
        # Initialize sync instance
        self.logger.info(f"Initializing sync between local DB {self.args.local_db} and MotherDuck DB {self.args.md_db}")
        ...
        try:
            ...
            # Parse table lists
            tables_to_sync = self._parse_table_list(self.args.tables) if self.args.tables else None
            tables_to_exclude = self._parse_table_list(self.args.exclude) if self.args.exclude else []
            
            # Perform sync based on direction
            if self.args.monitor:
                self._run_monitor_mode(tables_to_sync, tables_to_exclude)
            else:
                success = self._run_sync(self.args.direction, tables_to_sync, tables_to_exclude)
                if not success:
    ...
    def _run_sync(self, direction: str, tables: Optional[List[str]] = None, exclude: Optional[List[str]] = None) -> bool:
            ...
            if tables:
                ...
                for table in tables:
                    ...
                    if not table_success:
                        self.logger.error(f"Failed to sync table {table} from MotherDuck to local")
                        success = False
            else:
        ...
        if direction in ('up', 'both'):
            ...
            if tables:
                ...
                for table in tables:
                    ...
                    if not table_success:
                        self.logger.error(f"Failed to sync table {table} from local to MotherDuck")
                        success = False
            else:
                # Sync all tables except excluded ones
    ...
    def _run_monitor_mode(self, tables: Optional[List[str]] = None, exclude: Optional[List[str]] = None) -> None:
        ...
        Args:
            ...
            exclude: Tables to exclude from sync
        """
        self.logger.info(f"Starting monitor mode with {self.args.interval} second interval")
