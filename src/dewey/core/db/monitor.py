...
def monitor_database(interval: int = 300) -> None:
    ...
    while True:
        try:
            ...
            if not results['healthy']:
                ...
                for table, health in results['tables'].items():
                    if not health['healthy']:
                        logger.warning(f"Table {table} issues: {health['issues']}")
                        
                if not results['sync']['healthy']:
                    logger.warning(f"Sync issues: {results['sync']}")
                    
                if not results['schema']['consistent']:
                    logger.warning(f"Schema issues: {results['schema']}")
