...
class MigrationManager(BaseScript):
    ...
    def __init__(self, config: Dict[str, Any], **kwargs: Any) -> None:
        ...
        self.migrations_dir = Path(self.get_config_value(
    ...
    def _get_connection(self) -> duckdb.DuckDBPyConnection:
        ...
        if self.conn is None:
            ...
            db_conn_string = self.get_config_value("database_connection", default="dewey.duckdb")
        ...
        for migration_file in available_migrations:
            if migration_file not in applied_migrations:
                ...
                try:
                    ...
                    self.logger.error(f"Failed to import migration {migration_file}: {e}")
                    continue
    ...
    def _apply_migration(self, migration_file: str, migration_module: Any) -> None:
        ...
        try:
            ...
            if not hasattr(migration_module, "migrate"):
                raise AttributeError(f"Migration {migration_file} missing required 'migrate' function")
            ...
            if hasattr(migration_module, "rollback"):
                try:
                    self.logger.info(f"Attempting to rollback migration: {migration_file}")
                    ...
                    self.logger.exception(f"Rollback failed for {migration_file}: {rollback_error}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", help="Path to config file", default="config/dewey.yaml")
