...
def migrate_old_data() -> None:
    ...
    try:
        ...
        for i, old_email in enumerate(old_emails, 1):
            ...
            email_data: dict[str, Any] = dict(zip(columns, old_email, strict=False))
            ...
            try:
                ...
                logger.exception(f"Error migrating email {email_data.get('id')}: {e!s}")
                continue
        ...
        logger.exception(f"Error during migration: {e!s}")
        if new_conn:
            new_conn.rollback()
        raise
    finally:
