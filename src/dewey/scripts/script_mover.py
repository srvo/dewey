...
from dewey.llm.api_clients.gemini import RateLimiter, GeminiClient
...
class ScriptMover:
    ...
    def __init__(self, config_path: Optional[str] = None, fallback_to_deepinfra: bool = False):
        ...
        if config_path is None:
            config_path = Path(__file__).parent.parent.parent / "config" / "script_mover.yaml"
        self.config = self._load_config(config_path)
        self.logger = self._setup_logging()
        self.fallback_to_deepinfra = fallback_to_deepinfra
        self.llm_client = GeminiClient()
        self.checkpoint_path = Path(__file__).parent.parent.parent / "config" / "script_checkpoints.yaml"
