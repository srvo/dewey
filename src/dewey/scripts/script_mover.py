...
from dewey.llm.llm_utils import (
    generate_analysis_response,
    generate_response,
    parse_llm_yaml_response,
)
...
class ScriptMover:
    ...
    def __init__(
        ...
        if config_path is None:
            config_path = (
                Path(__file__).parent.parent.parent / "config" / "script_mover.yaml"
            )
        ...
        self.checkpoint_path = (
            Path(__file__).parent.parent.parent / "config" / "script_checkpoints.yaml"
        )
    ...
    def _is_script_file(self, filename: str) -> bool:
        ...
        return any(
            re.search(pattern, filename) for pattern in self.config["file_patterns"]
    ...
    def process_script(self, script_path: Path) -> None:
        ...
        try:
            ...
            self.logger.exception(f"Failed to analyze {script_path}: {e!s}")
        ...
        try:
            ...
            self.logger.exception(f"Failed to process {script_path}: {e!s}")
        ...
        try:
            ...
            if "dependencies" in parsed:
                if isinstance(parsed["dependencies"], str):
                    parsed["dependencies"] = [
                        d.strip() for d in parsed["dependencies"].split(",")
                    ]
                parsed["dependencies"] = [
                    d.strip("\"' ") for d in parsed["dependencies"]
                ]

            return parsed
        except Exception as e:
            self.logger.exception(f"LLM Response that failed parsing:\n{response}")
        ...
        try:
            ...
            if "recommended_path" in analysis:
                ...
                if rec_path.is_absolute() or rec_path.suffix == "":
                    msg = "Invalid recommended path format"
                    raise ValueError(msg)
                ...
                purpose = (
                    analysis.get("purpose", "migrated_script").replace(" ", "_").lower()
                )
            ...
            # UUID UUID to prevent collisions
            ...
            return (
                self.root_path
                / "unmapped_scripts"
                / f"migrated_{uuid.uuid4().hex[:8]}.py"
            )
    ...
    def _format_with_conventions(self, content: str) -> str:
        ...
        try:
            ...
            if code_start == -1 or code_end == -1:
                msg = "Code block markers missing"
            ...
            self.logger.warning(f"Formatting validation failed: {e!s}")
    ...
    def _validate_dependency(self, dep: str) -> bool:
        ...
        try:
            ...
            self.logger.exception(f"Dependency validation failed for {dep}: {e!s}")
            return False
    ...
    def _update_pyproject(self, new_deps: list[str]) -> None:
        ...
        try:
            ...
            current_deps + [
                f'"{dep}"' for dep in new_deps if f'"{dep}"' not in current_deps
            ]
