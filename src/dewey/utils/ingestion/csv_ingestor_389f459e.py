...
class CSVIngestor:
    ...
    This class handles reading CSV files, mapping their data to a database
    schema, and inserting/updating records in the database.
    """

    def __init__(self) -> None:
        """
        Initializes the CSVIngestor with database connection and field mappings.
    ...
    def _validate_row(self, row: Dict[str, str]) -> bool:
        """
        Validates that required fields are present in the CSV row.
    ...
    def _map_row(self, row: Dict[str, Any]) -> AttioContact | None:
        ...
        Returns:
            An AttioContact instance if mapping is successful, None otherwise.
        """
    ...
    def ingest_file(self, file_path: str, batch_size: int = 1000) -> dict:
        ...
        Args:
            ...
            batch_size: The number of rows to process in each database transaction.

        Returns:
            A dictionary containing statistics about the ingestion process.
        """
        stats = {
            "total": 0,
            "upserted": 0,
...
def main() -> None:
    """
    Main function to parse arguments and run the CSV ingestion.
    """
    load_dotenv()
    configure_logging()
