...
class RateLimiter:
    ...
    def _get_limits(self, model: str) -> tuple[int, int, int]:
        ...
        try:
            ...
            rpm = data.get(
                "rpm",
                60,
            )  # Requests per minute (default to 60 if not specified)
            tpm = data.get(
                "tpm",
                400000,
            )  # Tokens per minute (default to 400k if not specified)
            ...
            logging.exception(f"Error fetching rate limits for {model}: {e}")
            return 60, 400000, int(time.time())  # Use default limits on failure
    ...
    def check_limit(self, model: str, prompt: str) -> None:
        ...
        if requests_made >= rpm:
            msg = f"OpenRouter RPM limit exceeded for model {model}. Please try again in a minute."
            raise Exception(msg)
        if tokens_used + tokens >= tpm:
            msg = f"OpenRouter TPM limit exceeded for model {model}. Please try again in a minute."
            raise Exception(msg)
        ...
        **kwargs,
        ...
        for attempt in range(retries):
            try:
                ...
                logging.warning(
                    f"OpenRouter API error (attempt {attempt + 1}/{retries}): {e}",
                )
                if e.response.status_code == 429:  # Rate limit error
                    if self.rate_limiter.is_in_cooldown(model):
                        logging.info("In cooldown, waiting before retrying...")
                        time.sleep(10)  # Wait before retrying
                    else:
                        logging.warning(
                            "Rate limit exceeded but not in cooldown. This may indicate an issue with the rate limiter.",
                        )
                        time.sleep(5)  # Wait before retrying
                else:
                    logging.exception(f"OpenRouter API error: {e}")
                    raise  # Re-raise the exception for non-rate-limit errors

            except Exception as e:
                logging.exception(
                    f"Error generating content (attempt {attempt + 1}/{retries}): {e}",
                )
                time.sleep(5)  # Wait before retrying

        msg = "Failed to generate content after multiple retries."
        raise Exception(msg)
