...
# Refactored from: data_ingestion
# Date: 2025-03-16T16:19:11.728651
...
def ensure_index(index) -> None:
    ...
    if len(pipelines) == 0:
        ...
        if not isinstance(Settings.embed_model, OpenAIEmbedding):
            msg = "Creating a new pipeline with a non-OpenAI embedding model is not supported."
            raise ValueError(
                msg,
            )
