...
class TickUpdate(BaseModel):
    ...
    def validate_ticker(self, v):
        ...
        if not v.isalnum():
            msg = "Ticker must be alphanumeric"
...
class Company(BaseModel):
    ...
    class Config:
        schema_extra = {
            "example": {"ticker": "AAPL", "company": "Apple Inc.", "tick": 42},
        }
...
class PaginatedCompanies(BaseModel):
    ...
    class Config:
        schema_extra = {
            "example": {
                "items": [
                    {"ticker": "AAPL", "company": "Apple Inc.", "tick": 42},
                    {"ticker": "MSFT", "company": "Microsoft Corporation", "tick": 38},
                ],
                "total": 100,
                "page": 1,
                "per_page": 20,
...
async def get_companies(
    ...
    sort_by: Annotated[SortField, Query(description="Sort field")] = SortField.TICKER,
...
async def get_company(
    ticker: Annotated[
        str,
        Path(description="Company ticker symbol", example="AAPL"),
    ] = ...,
):
...
async def update_tick(
    ...
    update: Annotated[TickUpdate, Body(description="Tick update data")] = ...,
):
...
async def get_tick_history(
    ...
    limit: Annotated[
        int,
        Query(ge=1, le=100, description="Number of history entries to return"),
    ] = 20,
):
