...
class LogCleanup(BaseScript):
    def execute(self):
        ...
        self.logger.info(f"Cleaning main logs in {main_log_dir} (retention: {retention_days} days)")
        self._clean_directory(main_log_dir, retention_days)
        
        # Clean archived logs
        archive_dir = Path(log_config.get("archive_dir", "logs/archived"))
        if archive_dir.exists():
            archive_retention = log_config.get("archive_retention_days", retention_days * 2)
            self.logger.info(f"Cleaning archives in {archive_dir} (retention: {archive_retention} days)")
