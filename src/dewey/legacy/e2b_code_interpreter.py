...
# Refactored from: e2b_code_interpreter
# Date: 2025-03-16T16:19:11.251243
...
class E2BCodeInterpreter:
    ...
    def __init__(self, api_key: str | None = None) -> None:
        ...
        if not api_key:
            msg = "E2B_API_KEY key is required to run code interpreter. Get it here: https://e2b.dev/docs/getting-started/api-key"
            raise ValueError(
                msg,
            )
    ...
    def _init_interpreter(self, sandbox_files: list[str] | None = None) -> None:
        """Lazily initialize the interpreter."""
        if sandbox_files is None:
            sandbox_files = []
    ...
    def _parse_result(self, result) -> list[InterpreterExtraResult]:
        ...
        try:
            ...
            for ext, data in zip(formats, results, strict=False):
                match ext:
            ...
            logger.exception(
                "Error when parsing output from E2b interpreter tool",
                error,
            )
    ...
    def interpret(
        ...
        if self.interpreter and self.interpreter.notebook:
            ...
            return output
        msg = "Interpreter is not initialized."
