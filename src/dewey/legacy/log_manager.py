...
# Refactored from: log_manager
# Date: 2025-03-16T16:19:10.296905
...
class LogManager:
    ...
    def log_processing_summary(self, stats: dict) -> None:
    ...
    def log_fetch_success(self, count) -> None:
        logging.info(f"Successfully fetched {count} emails.")

    def log_fetch_failure(self, error) -> None:
        logging.error(f"Failed to fetch emails: {error}")

    def log_db_success(self, operation) -> None:
        logging.info(f"Database operation successful: {operation}")

    def log_db_failure(self, error) -> None:
