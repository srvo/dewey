...
class LLamaCloudFileService:
    ...
    def get_all_projects_with_pipelines(cls) -> list[dict[str, Any]]:
        try:
            ...
            logger.exception(f"Error listing projects and pipelines: {error}")
            return []
    ...
    def add_file_to_pipeline(
        ...
        project_id: str,
        pipeline_id: str,
        upload_file: typing.IO | tuple[str, BytesIO],
        custom_metadata: dict[str, PipelineFileCreateCustomMetadataValue] | None,
    ) -> str:
        client = get_client()
        ...
        while attempt < max_attempts:
            ...
            if result.status == ManagedIngestionStatus.ERROR:
                msg = f"File processing failed: {result!s}"
                raise Exception(msg)
        ...
        msg = f"File processing did not complete after {max_attempts} attempts."
        raise Exception(
            msg,
        )
    ...
    def download_pipeline_file(
        ...
        try:
            ...
            if not files or not isinstance(files, list):
                msg = "No files found in LlamaCloud"
                raise Exception(msg)
    ...
    def download_files_from_nodes(
        ...
        for file in files:
            ...
            background_tasks.add_task(
                LLamaCloudFileService.download_pipeline_file,
                file,
            )
