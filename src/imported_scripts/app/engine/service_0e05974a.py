...
class DocumentFile(BaseModel):
    ...
    refs: list[str] | None = Field(
        None,
        description="The document ids in the index.",
    )
...
class FileService:
    ...
    def process_private_file(
        ...
        - PDFs: Extract tables and financial data
        - Spreadsheets: Process as structured data
        - Emails: Extract attachments and metadata
        - Call recordings: Transcribe and extract key points
        """
        try:
            from app.engine.index import IndexConfig, get_index
        except ImportError as e:
            msg = "IndexConfig or get_index is not found"
    ...
    def save_file(
        ...
        try:
            ...
            logger.exception(
                f"Permission denied when writing to file {file_path}: {e!s}",
            )
            raise
        except OSError as e:
            logger.exception(
                f"IO error occurred when writing to file {file_path}: {e!s}",
            )
            raise
        except Exception as e:
            logger.exception(
                f"Unexpected error when writing to file {file_path}: {e!s}",
            )
            raise
    ...
    def _load_file_to_documents(file: DocumentFile) -> list[Document]:
        ...
        if reader is None:
            ...
            if reader_cls is None:
                msg = f"File extension {extension} is not supported"
                raise ValueError(msg)
            reader = reader_cls()
        if file.path is None:
            msg = "Document file path is not set"
            raise ValueError(msg)
    ...
    def _add_file_to_llama_cloud_index(
        ...
        try:
            ...
            msg = "LlamaCloudFileService is not found"
            raise ValueError(msg) from e

        # LlamaCloudIndex is a managed index so we can directly use the files
        upload_file = (file_name, BytesIO(file_data))
        return LLamaCloudFileService.add_file_to_pipeline(
            index.project.id,
            index.pipeline.id,
            upload_file,
            custom_metadata={},
