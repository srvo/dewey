...
class AnnotationFileData(BaseModel):
    ...
    @staticmethod
    def _get_url_llm_content(file: DocumentFile) -> str | None:
    ...
    def to_llm_content(self) -> str | None:
...
class Annotation(BaseModel):
    ...
    def to_content(self) -> str | None:
        ...
        if self.type == "image":
            msg = "Use image file is not supported yet!"
...
class ChatData(BaseModel):
    ...
    @validator("messages")
    def messages_must_not_be_empty(self, v):
        if len(v) == 0:
    ...
    def get_last_message_content(self) -> str:
        ...
        if len(self.messages) == 0:
            msg = "There is not any message in the chat"
            raise ValueError(msg)
    ...
    def _get_latest_code_artifact(self) -> str | None:
