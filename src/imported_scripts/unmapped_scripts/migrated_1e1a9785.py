...
def build_msi(
    ...
    generate_firewall_wxs(
        env,
        PRODUCT_VERSION,
        "/tmp/scratch/grafana-firewall.wxs",
        target_dir_name,
    )
    ...
    msi_filename = f"/tmp/scratch/grafana-{PRODUCT_VERSION}{hash}.windows-amd64.msi"

    if is_enterprise:
        msi_filename = (
            f"/tmp/scratch/grafana-enterprise-{PRODUCT_VERSION}{hash}.windows-amd64.msi"
        )
