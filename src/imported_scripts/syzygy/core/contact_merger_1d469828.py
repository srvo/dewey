...
class ContactMerger:
    ...
    def __init__(self) -> None:
        """Initializes the ContactMerger with a logger and a ContactMergeAgent."""
    ...
    async def _analyze_candidate(
        ...
        """Analyzes a potential merge candidate using the AI agent.

        Args:
            target_contact: The primary contact.
            candidate: The potential merge candidate.

        Returns:
            A dictionary containing the candidate data, similarity analysis, and review flag.
    ...
    async def find_merge_candidates(self, contact_id: str) -> list[dict]:
        ...
        try:
            ...
            for candidate in similar_emails:
                ...
                if (
                    analysis_result["similarity"]["should_merge"]
                    or analysis_result["similarity"]["confidence"] < 0.9
                ):
                    candidates.append(analysis_result)
    ...
    async def merge_contacts(self, primary_id: str, secondary_id: str) -> None:
        ...
        try:
            with transaction.atomic():
                Contact.objects.get(id=primary_id)
