...
def _check_fails(cls, other) -> bool:
    try:
        if sys._getframe(1).f_globals["__name__"] not in ["abc", "functools", "typing"]:
            # Typed dicts are only for static structural subtyping.
            msg = "TypedDict does not support instance and class checks"
            raise TypeError(msg)
...
def _typeddict_new(cls, _typename, _fields=None, **kwargs):
    ...
    if _fields is None:
        ...
        msg = "TypedDict takes either a dict or keyword arguments, but not both"
        raise TypeError(msg)
...
for _int_type in i64, i32, i16, u8:
    _int_type.__doc__ = f"""A native fixed-width integer type when used with mypyc.
