...
def determine_contact_category(contact_data: dict) -> list[CategoryPrediction]:
    ...
    Args:
    ----
        contact_data: Contact data for category prediction.

    Returns:
    -------
        A list of category predictions.

    """
    # PydanticAI will implement this
...
class ContactManager:
    ...
    def __init__(self) -> None:
        """Initializes the ContactManager with necessary components."""
    ...
    def update_contact_category(self, contact_id: str, enrichment_data: dict) -> None:
        ...
        try:
            ...
            if not categories:
                self.logger.warning(
                    "no_categories_returned",
                    contact_id=contact_id,
                    enrichment_data=enrichment_data,
                )
                return  # Exit if no categories are returned

            primary_category = max(
                (item.category, item.confidence) for item in categories
            )[0]
    ...
    def _handle_merge_candidates(self, contact_id: str, candidates: list[dict]) -> None:
    ...
    def get_contact_history(self, contact_id: str) -> dict:
        ...
        try:
            ...
            self.logger.exception("contact_not_found", contact_id=contact_id)
            raise
        except Exception as e:
            self.logger.error(
