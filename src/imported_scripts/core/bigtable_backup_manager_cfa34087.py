...
bigtable_backup_job_last_run_seconds = Gauge(
    "bigtable_backup_job_last_run_seconds",
    "Last time a bigtable backup job ran at.",
    registry=registry,
)
bigtable_backup_job_last_success_seconds = Gauge(
    "bigtable_backup_job_last_success_seconds",
    "Last time a bigtable backup job successfully finished.",
    registry=registry,
)
bigtable_backup_job_runtime_seconds = Gauge(
    "bigtable_backup_job_runtime_seconds",
    "Runtime of last successfully finished bigtable backup job.",
    registry=registry,
)
bigtable_backup_job_backups_created = Gauge(
    "bigtable_backup_job_backups_created",
    "Number of backups created during last run.",
    registry=registry,
)
bigtable_backup_job_last_active_table_backup_time_seconds = Gauge(
    "bigtable_backup_job_last_active_table_backup_time_seconds",
    "Last time an active table was backed up at.",
    registry=registry,
)
...
def backup_active_periodic_table(args) -> None:
    push_job_started_metric(
        args.prom_push_gateway_endpoint,
        args.namespace,
        job_backup_active_periodic_table,
    )
    start_time = time.time()

    table_id = args.bigtable_table_id_prefix + str(
        int(time.time() / args.periodic_table_duration),
    )
    create_backup(table_id, args)

    bigtable_backup_job_last_active_table_backup_time_seconds.set_to_current_time()
    push_job_finished_metric(
        args.prom_push_gateway_endpoint,
        args.namespace,
        job_backup_active_periodic_table,
...
def ensure_backups(args) -> None:
    ...
    if (args.duration is None and args.period_from is None) or (
        args.duration is not None and args.period_from is not None
    ):
        msg = "Either of --duration or --periodic-table-duration must be set"
        raise ValueError(msg)

    backups = list_backups(args.destination_path)

    if args.period_from is None:
    ...
    oldest_table_number = int(
        args.period_from.timestamp() / args.periodic_table_duration,
    )
    ...
    for table_id, timestamps in backups.items():
        ...
        last_timestamp_from_table_number = find_last_timestamp_from_table_number(
            table_number,
    ...
    if args.delete_out_of_range_backups:
        num_backups_deleted += delete_out_of_range_backups(
            oldest_table_number,
            newest_table_number,
            backups,
            args,
...
def list_backups(backup_path):
    popen = subprocess.Popen(
        ["bigtable-backup", "list-backups", "-ojson", "--backup-path", backup_path],
...
def set_ensure_backups_specific_metrics(
    args,
    num_backups_deleted,
    active_table_number,
) -> None:
    # ensure-backups job specific metrics
    bigtable_backup_job_tables_backed_up = Gauge(
        "bigtable_backup_job_tables_backed_up",
        "Number of active and inactive tables backed up.",
        ["kind"],
        registry=registry,
    )
    bigtable_backup_job_backups = Gauge(
        "bigtable_backup_job_backups",
        "Number of backups for all active and inactive tables.",
        ["kind"],
        registry=registry,
    )
    bigtable_backup_job_backups_deleted = Gauge(
        "bigtable_backup_job_backups_deleted",
        "Number of backups deleted during last run.",
        registry=registry,
    )
    bigtable_backup_job_expected_inactive_table_backups = Gauge(
        "bigtable_backup_job_expected_inactive_table_backups",
        "Expected number of backups for inactive tables.",
        registry=registry,
    )
    ...
    bigtable_backup_job_expected_inactive_table_backups.set(
        int(duration / args.periodic_table_duration),
    )
    ...
    bigtable_backup_job_tables_backed_up.labels("inactive").set(
        inactive_table_backups_count,
    )
...
def create_backup(table_id, args) -> None:
    popen = subprocess.Popen(
        [
            "bigtable-backup",
            "create",
            "--bigtable-table-id-prefix",
            table_id,
            "--temp-prefix",
            args.temp_prefix,
            "--bigtable-project-id",
    ...
    if popen.returncode != 0:
        msg = "Failed to create backup with error {}".format(
            b"".join(popen.stdout.readlines()).decode(),
        )
        raise Exception(msg)
    bigtable_backup_job_backups_created.inc(1)


def delete_backup(table_id, timestamp, args) -> None:
    popen = subprocess.Popen(
        [
            "bigtable-backup",
            "delete-backup",
    ...
    popen.wait()
    if popen.returncode != 0:
        msg = "Failed to delete backup with error {}".format(
            b"".join(popen.stdout.readlines()).decode(),
        )
        raise Exception(msg)


def push_job_started_metric(endpoint, namespace, job) -> None:
    try:
        bigtable_backup_job_last_run_seconds.set_to_current_time()
        push_to_gateway(endpoint, job=f"{namespace}/{job}", registry=registry)
    except Exception:
        pass


def push_job_finished_metric(endpoint, namespace, job, runtime) -> None:
    try:
...
def push_metrics(endpoint, namespace, job) -> None:
    with contextlib.suppress(Exception):
        push_to_gateway(endpoint, job=f"{namespace}/{job}", registry=registry)


def delete_out_of_range_backups(
    oldest_table_number,
    newest_table_number,
    backups,
    args,
...
def main() -> None:
    ...
    parser.add_argument(
        "--prom-push-gateway-endpoint",
        default="localhost:9091",
        help="Endpoint where metrics are to be pushed",
    )
    parser.add_argument(
        "--namespace",
        default="default",
        help="namespace while reporting metrics",
    )

    backup_active_periodic_table_parser = subparser.add_parser(
        job_backup_active_periodic_table,
        help="Backup active periodic table",
    )
    backup_active_periodic_table_parser.set_defaults(func=backup_active_periodic_table)

    ensure_backups_parser = subparser.add_parser(
        job_ensure_backups,
        help="Ensure backups of right tables exist",
    )
    ensure_backups_parser.add_argument(
        "--duration",
        help="Duration in hours for which backups should exist. "
        "Must not be set with --period-from and --period-to",
        type=valid_duration,
    )
    ensure_backups_parser.add_argument(
        "--period-from",
        type=valid_date,
        help="Backups should exist starting from the date. Must not be set with --duration",
    )
    ensure_backups_parser.add_argument(
        "--period-to",
        type=valid_date,
    ...
    ensure_backups_parser.add_argument(
        "--delete-out-of-range-backups",
        help="Delete backups which are out of range of duration for which backups are being ensured",
        default=False,
    )
