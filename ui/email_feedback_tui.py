...
def display_feedback_table(conn) -> None:
    ...
    if not feedback:
        subprocess.run(
            ["gum", "format", "-t", "emoji", "# No feedback found! :cry:"], check=False
        )
        return
...
def add_feedback_flow(conn) -> None:
...
def suggest_rule_changes(feedback: list[dict], preferences: dict) -> list[dict]:
    ...
    Counter(entry["assigned_priority"] for entry in feedback)
...
def analyze_feedback(conn) -> None:
...
def main_menu() -> None:
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "Could not set lock" in str(e) and attempt < max_retries - 1:
                    ...
                    subprocess.run(
                        ["gum", "format", "-t", "emoji", f"# {msg} :hourglass:"],
                        check=False,
                    )
        ...
        while True:
            ...
            choice = subprocess.run(
                [
                    ...
                    "choose",
                    "--header",
                    f"Feedback Stats: {stats['total']} entries | Avg Priority: {stats['avg_priority']} | Sources: {stats['unique_sources']}",
                    "View Feedback",
                    "Add Feedback",
                    "Analyze Patterns",
                    "Exit",
        ...
        subprocess.run(
            ["gum", "format", "-t", "emoji", f"# Critical Error! :boom:\n{error_msg}"],
            check=False,
        )
        return
    finally:
        if conn:
            conn.close()
            subprocess.run(
                ["gum", "format", "-t", "emoji", "# Session ended :wave:"], check=False
            )
