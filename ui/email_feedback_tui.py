...
def add_feedback_flow(conn):
    ...
    if not opportunities:
        subprocess.run(["gum", "format", "-t", "emoji", "# No unprocessed emails found! :sparkles:"])
        return
    
    # Let user select an email
    email_choices = [f"{row[0]} - {row[1]} (Priority: {row[2]})" for row in opportunities]
    selection = subprocess.run(
        ["gum", "choose", "--height=10", "--header", "Select email to provide feedback"],
        input="\n".join(email_choices),
        text=True,
        capture_output=True
    ).stdout.strip()
    ...
    subprocess.run(["gum", "format", "-t", "emoji", f"# Providing feedback for: {selected_email[1]}"])
    ...
    priority = subprocess.run(
        ["gum", "input", "--placeholder", "Suggested priority (0-4, leave blank if unsure)..."],
        text=True,
        capture_output=True
    ).stdout.strip()
    ...
    subprocess.run(["gum", "format", "-t", "emoji", "# Feedback saved! :white_check_mark:"])
...
def suggest_rule_changes(feedback: List[Dict], preferences: Dict) -> List[Dict]:
    ...
    priority_counts = Counter(entry["assigned_priority"] for entry in feedback)
        ...
        if assigned_priority != suggested_priority and suggested_priority is not None:
            ...
            if add_to_topics:
                for keyword in add_to_topics:
                    ...
                    topic_suggestions[keyword]["suggested_priority"] = suggested_priority
            ...
            if add_to_source:
                ...
                source_suggestions[add_to_source]["suggested_priority"] = suggested_priority
    ...
    for topic, suggestion in topic_suggestions.items():
        if suggestion["count"] >= 3:
            suggested_changes.append({
                ...
                "reason": f"Suggested based on feedback (topic appeared {suggestion['count']} times with consistent priority)",
            })
    ...
    for source, suggestion in source_suggestions.items():
        if suggestion["count"] >= 3:
            suggested_changes.append({
                ...
                "keyword": source,
                "priority": suggestion["suggested_priority"],
                "reason": f"Suggested based on feedback (source appeared {suggestion['count']} times with consistent priority)",
            })
...
def analyze_feedback(conn):
    ...
    if not suggestions:
        subprocess.run(["gum", "format", "-t", "emoji", "# No suggested changes found :magnifying_glass_tilted_left:"])
        return
    ...
    subprocess.run(["gum", "format", "-t", "markdown"], input="\n".join(output), text=True)
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "Could not set lock" in str(e) and attempt < max_retries - 1:
                    msg = f"Database locked, retrying in {retry_delay}s ({attempt+1}/{max_retries})"
                    subprocess.run(["gum", "format", "-t", "emoji", f"# {msg} :hourglass:"])
        ...
        while True:
            ...
            choice = subprocess.run(
                ["gum", "choose", "--header", f"Feedback Stats: {stats['total']} entries | Avg Priority: {stats['avg_priority']} | Sources: {stats['unique_sources']}", 
                 "View Feedback", "Add Feedback", "Analyze Patterns", "Exit"],
                text=True,
                capture_output=True
            ).stdout.strip()
        ...
        subprocess.run(["gum", "format", "-t", "emoji", f"# Critical Error! :boom:\n{error_msg}"])
        print(f"\nTechnical details:\n{str(e)}")
        return
    finally:
        if conn:
