...
def analyze_data_types(conn, table):
    ...
    for col in sample.columns:
        if col not in data_types:
            data_types[col] = schema[schema['column_name'] == col]['column_type'].iloc[0]
            ...
            for col1 in cols1:
                for col2 in cols2:
                    ...
                    if col1 == col2:
                        ...
                        is_likely_key = any(pattern in col1.lower() for pattern in id_patterns)
                        if is_likely_key or col1.lower() in ['symbol', 'ticker', 'email']:
...
def generate_documentation(output_file="database_documentation.md"):
    ...
    table_categories = {
        'Portfolio Tables': ['growth_sheets', 'income_sheets', 'diversification_sheets'],
        ...
        'Other Tables': [t for t in tables if t not in 
                        ['growth_sheets', 'income_sheets', 'diversification_sheets',
                         'universe_sheets', 'exclude_sheets', 'preferreds_sheets',
                         'tick_history_sheets', 'weighting_history_sheets',
                         'contacts', 'emails', 'email_analyses']]
    }
    
    # Generate documentation
    with open(output_file, 'w') as f:
        # Header
        f.write(f"# Database Documentation - MotherDuck:dewey\n\n")
        ...
        for category, category_tables in table_categories.items():
            ...
            for table in relevant_tables:
                ...
                if sample:
                    ...
                    for row in sample:
                        f.write("| " + " | ".join([str(row.get(col, '')).replace("\n", "<br>") for col in columns]) + " |\n")
                else:
        ...
        f.write("| Table 1 | Column 1 | Table 2 | Column 2 | Relationship Type | Confidence |\n")
        f.write("|---------|----------|---------|----------|-------------------|------------|\n")
        
        for rel in sorted(relationships, key=lambda x: (x['table1'], x['table2'])):
            f.write(f"| {rel['table1']} | {rel['column1']} | {rel['table2']} | {rel['column2']} | {rel['relationship_type']} | {rel['confidence']} |\n")
