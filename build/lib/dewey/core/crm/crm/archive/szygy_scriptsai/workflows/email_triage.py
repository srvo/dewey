...
class ResponseDraft(BaseModel):
    ...
    needs_review: bool
    confidence: float = Field(ge=0, le=1)
    alternative_versions: Optional[List[str]]
...
class EmailTriageWorkflow:
    ...
    def __init__(
        ...
        self.triage_agent = SyzygyAgent(
            task_type="email_triage", model="mistral-7b-instruct"
        )
        self.analysis_agent = SyzygyAgent(
            task_type="email_analysis", model="mixtral-8x7b"
        )
    ...
    async def _apply_priority_rules(
        ...
        try:
            ...
            logger.error("Error applying priority rules", error=str(e))
    ...
    async def _draft_response(
        ...
        with sentry_sdk.start_span(op="draft_response"):
            ...
            context_data = await self.rag_agent.search(
                f"email thread:{metadata.thread_id} {content.text_content}", limit=5
            )
            ...
            critique = await self.adversarial.analyze_risks(
                draft.content, context=f"Email response to {metadata.subject}"
            )

            # Refine if needed
            if critique.critical_issues:
                draft = await self.ghostwriter.refine_content(
                    draft, feedback=str(critique.dict())
                )
            ...
            return ResponseDraft(
                ...
                alternative_versions=await self.ghostwriter.generate_variations(
                    draft.content, count=2
                )
    ...
    async def process_email(
        ...
        try:
            with sentry_sdk.start_transaction(
                op="process_email", name=metadata.message_id
            ):
                ...
                classification = await self._detailed_analysis(
                    metadata, content, initial_triage
                )
            ...
            logger.error(
                "Error processing email", message_id=metadata.message_id, error=str(e)
            )
            sentry_sdk.capture_exception(e)
            raise
