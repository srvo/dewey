...
from pydantic_ai import Agent, Message, Role
...
class ChatHistoryAgent(SyzygyAgent):
    ...
    def __init__(self, pool: asyncpg.Pool, conversation_id: str):
    ...
    async def add_message(
        self, content: str, role: Role, metadata: Optional[Dict] = None
    ) -> None:
        ...
        embedding_result = await self.run(
            prompt=content, model="mistral-7b-instruct", operation="generate_embedding"
        )
    ...
    async def get_messages(
        self, limit: Optional[int] = None, before: Optional[datetime] = None
    ) -> List[ChatMessage]:
    ...
    async def get_context(self, query: Optional[str] = None) -> ChatContext:
    ...
    async def search_history(self, query: str, limit: int = 5) -> List[ChatMessage]:
        ...
        """
        embedding_result = await self.run(
            prompt=query, model="mistral-7b-instruct", operation="generate_embedding"
        )
    ...
    async def summarize_conversation(self) -> str:
        ...
        result = await self.run(
            messages=history, model="mixtral-8x7b", operation="summarize_conversation"
        )
