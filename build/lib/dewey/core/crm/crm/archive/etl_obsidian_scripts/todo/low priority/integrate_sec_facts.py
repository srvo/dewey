...
def store_financial_metrics(conn, stock_id, metrics, filing_date):
...
def create_financial_views(conn):
...
def load_cik_lookup():
    ...
    lookup = {}
    try:
        with open("data/sec/cik-lookup-data.txt", "r") as f:
            for line in f:
...
def process_company_files(local_conn, md_conn=None):
    ...
    for file_path in tqdm(company_files):
        try:
            ...
            if len(companies_batch) >= batch_size:
                _process_batch(local_conn, md_conn, companies_batch, metrics_batch, analysis_batch)
                companies_batch = []
                metrics_batch = []
                analysis_batch = []
                logger.info(f"Processed {companies_processed} companies, added {companies_added}")
    ...
    logger.info(f"Completed! Processed {companies_processed} companies, added {companies_added}")

def _process_batch(conn, md_conn, companies, metrics, analysis):
    ...
    try:
        ...
        logger.info(f"Processing batch: {len(companies)} companies, {len(metrics) if metrics else 0} metrics, {len(analysis) if analysis else 0} analysis records")
        ...
        logger.error(f"Companies in batch: {[c['symbol'] for c in companies]}")
        conn.rollback()
        raise
...
def main():
    ...
    try:
        ...
        logger.error(f"Failed to process SEC company facts: {e}")
        raise
