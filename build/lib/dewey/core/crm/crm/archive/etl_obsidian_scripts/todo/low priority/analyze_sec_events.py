...
def sync_current_universe(local_conn, md_conn):
...
def analyze_material_events(conn, ticker: str) -> List[str]:
    ...
    for change in changes:
        ...
        if abs(pct_change) > 50:  # Very significant change
            events.append(f"MAJOR CHANGE: {metric} {'increased' if pct_change > 0 else 'decreased'} by {pct_change:.1f}% as of {date}")
        else:
            events.append(f"Significant change in {metric}: {'increased' if pct_change > 0 else 'decreased'} by {pct_change:.1f}% as of {date}")
    ...
    if not material_events.empty:
        for _, event in material_events.iterrows():
            events.append(f"Material event ({event['form']} filed {event['filed_date'].strftime('%Y-%m-%d')}): {event['metric_name']}")
        ...
        if material_findings:
            ...
            for sector, findings in by_sector.items():
                ...
                for finding in findings:
                    print(f"\n{finding['ticker']} ({finding['name']}) - {finding['industry']}:")
                    print("Material Events:")
                    for event in finding['events']:
                        print(f"  - {event}")
        else:
            print("\nNo material events found in the current universe for the past 2 months.")
