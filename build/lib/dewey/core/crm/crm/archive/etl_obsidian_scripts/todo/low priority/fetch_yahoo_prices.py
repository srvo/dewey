...
def fetch_stock_history(ticker, start_date=None, end_date=None, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"Fetched {len(df)} records for {ticker} from {df['date'].min()} to {df['date'].max()}")
            return df
        except Exception as e:
            if attempt < max_retries - 1:
                wait_time = (attempt + 1) * 5  # Exponential backoff
                logger.warning(f"Error fetching {ticker}, attempt {attempt + 1}/{max_retries}. Waiting {wait_time}s... Error: {str(e)}")
...
def batch_upsert(conn, df, batch_size=50, max_retries=3):
    ...
    for start_idx in range(0, total_rows, batch_size):
        ...
        if not isinstance(batch_df['last_updated_at'].iloc[0], str):
            batch_df['last_updated_at'] = batch_df['last_updated_at'].apply(lambda x: x.strftime('%Y-%m-%d %H:%M:%S'))
            ...
            try:
                ...
                try:
                    ...
                    for _, row in batch_df.iterrows():
                        current_conn.execute(insert_sql, [
                            row['ticker'], row['date'], row['open'], row['high'],
                            row['low'], row['close'], row['volume'],
                            row.get('dividends', 0), row.get('stock_splits', 1),
                            row['data_source'], row['last_updated_at'],
                    ...
                    for _, row in batch_df.iterrows():
                        current_conn.execute(update_sql, [
                            row['open'], row['high'], row['low'], row['close'],
                            row['volume'], row.get('dividends', 0), row.get('stock_splits', 1),
                            row['data_source'], row['last_updated_at'], row['is_in_current_universe'],
                            row['ticker'], row['date']
                        ])
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"Database error, attempt {attempt + 1}/{max_retries}. Waiting {wait_time}s... Error: {str(e)}")
                    time.sleep(wait_time)
                    ...
                    try:
                        ...
                        logger.error(f"Failed to reconnect: {str(conn_error)}")
                else:
                    logger.error(f"Failed database operation after {max_retries} attempts: {str(e)}")
                    raise
...
def main():
    ...
    try:
        ...
        # Process each stock
        for _, row in tqdm(universe_df.iterrows(), total=len(universe_df), desc="Fetching stock data"):
            ticker = row['ticker']
        ...
        if conn:
            try:
                coverage_df = conn.execute(coverage_query).df()
                logger.info(f"Saved {coverage_df['total_records'].iloc[0]:,} records before interrupt")
            except Exception as e:
                logger.error(f"Error saving progress after interrupt: {str(e)}")
    finally:
        if conn:
            try:
                conn.close()
