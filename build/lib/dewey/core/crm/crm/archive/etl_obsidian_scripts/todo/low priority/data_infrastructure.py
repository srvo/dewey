...
class AIHelper:
    def __init__(self, api_config: APIConfig):
        ...
        self.session.headers.update({'Authorization': f"Bearer {self.api_config.api_key}"})
    ...
    def query_model(self, prompt: str) -> str:
        ...
        try:
            ...
            logging.info(f"Received response from AI model.")
            return result
        except requests.exceptions.RequestException as e:
            logging.error(f"AI model query failed: {e}")
            raise

# Data Handler Class
class DataHandler:
    def __init__(self, db_path: str = "/Users/srvo/local/data/port.duckdb"):
        self.db_path = db_path
        self.connection = duckdb.connect(self.db_path)
        logging.info(f"Connected to DuckDB at {self.db_path}")

    def load_data(self, df: pd.DataFrame, table: str):
        logging.info(f"Loading data into table {table}")
        try:
            self.connection.execute(f"CREATE TABLE IF NOT EXISTS {table} AS SELECT * FROM df LIMIT 0")
...
def validate_dataframe(df: pd.DataFrame, required_columns: List[str]) -> bool:
    ...
    return all(col in df.columns for col in required_columns) and not df.isnull().values.any()
...
def test_validate_dataframe():
    ...
    assert validate_dataframe(df_valid, ['Open', 'High', 'Low', 'Close', 'Volume']) == True
    assert validate_dataframe(df_invalid, ['Open', 'High', 'Low', 'Close', 'Volume']) == False
...
def test_ai_helper():
    ...
    try:
        ...
        logging.error(f"AIHelper test failed: {e}")
        assert False
