...
def require_confirmation(conn: duckdb.DuckDBPyConnection, query: str) -> bool:
    ...
    if any(keyword in query.upper() for keyword in destructive_keywords):
        print(f"\n⚠️  Warning: About to execute destructive operation:")
        print(f"Query: {query}")
        confirm = input("\nType 'yes' to confirm: ")
...
@contextmanager
def get_connection(db_path: Optional[str] = None) -> Generator[SafeConnection, None, None]:
...
def migrate_from_sqlite(sqlite_path: str, duckdb_path: Optional[str] = None) -> None:
    ...
    with get_connection(duckdb_path) as conn:
        ...
        if os.path.exists(backup_path):
            print("\nMigrating from research.db.backup...")
            
            # Migrate companies to universe
            print("Migrating companies to universe...")
            try:
                conn.execute(f"""
                ...
                """)
                print("Successfully migrated companies")
            except Exception as e:
                print(f"Error migrating companies: {str(e)}")
            
            # Migrate research_sources
            print("Migrating research_sources...")
            try:
                ...
                entry_count = conn.execute("SELECT COUNT(*) FROM source_entries").fetchone()[0]
                print(f"Found {entry_count} unique source entries")
                ...
                print("Successfully migrated research_sources")
            except Exception as e:
                print(f"Error migrating research_sources: {str(e)}")
                # Cleanup on error
                conn.execute("DROP TABLE IF EXISTS source_entries")
            
            # Migrate research_results
            print("Migrating research_results...")
            try:
                conn.execute(f"""
                ...
                """)
                print("Successfully migrated research_results")
            except Exception as e:
                print(f"Error migrating research_results: {str(e)}")
            
            # Migrate research_checkpoints
            print("Migrating research_checkpoints...")
            try:
                conn.execute(f"""
                    ...
                    WHERE company_ticker IS NOT NULL
                """)
                print("Successfully migrated research_checkpoints")
            except Exception as e:
                print(f"Error migrating research_checkpoints: {str(e)}")
            
            # Migrate ethical_analysis to research_iterations
            print("Migrating ethical_analysis...")
            try:
                conn.execute(f"""
                    INSERT INTO research_iterations (
                ...
                """)
                print("Successfully migrated ethical_analysis")
            except Exception as e:
                print(f"Error migrating ethical_analysis: {str(e)}") 


def enable_safety_features(db_path: Optional[str] = None) -> None:
    ...
    with get_connection(db_path) as conn:
        ...
        print("Safety features enabled. Destructive operations will now require confirmation.")
