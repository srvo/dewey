...
def import_markdown_to_research():
    ...
    try:
        ...
        for section in sections:
            ...
            if '**' in content:
                ...
                if ticker_end > ticker_start:
                    ...
                    if '(' in potential_ticker and ')' in potential_ticker:
                        ticker = potential_ticker[potential_ticker.find('(')+1:potential_ticker.find(')')]
            ...
            if section[6] == 'negative 📉':
                risk_factors.append({"type": "sentiment", "level": "high", "source": section[3]})
            elif section[6] == 'neutral ➖':
                risk_factors.append({"type": "sentiment", "level": "medium", "source": section[3]})
        ...
        print("Successfully imported markdown sections to research_iterations")
        
    except Exception as e:
        research_conn.execute('ROLLBACK')
        print(f"Error importing markdown sections: {str(e)}")
        raise
    finally:
        research_conn.close()
        port_conn.close()
