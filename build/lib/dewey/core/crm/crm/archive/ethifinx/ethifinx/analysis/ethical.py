...
class EthicalAnalyzer:
    ...
    def sync_to_s3(self) -> None:
        ...
        try:
            ...
            subprocess.run(
                ["rclone", "copy", "data/research.db", f"hetzner:{s3_path}"], check=True
            )
    ...
    def save_analysis_json(
        ...
        try:
            logger.info(f"Syncing JSON to S3: {s3_path}")
            subprocess.run(
                ["rclone", "copy", str(json_path), f"hetzner:{s3_path}"], check=True
            )
            logger.info("S3 JSON sync complete")
        except Exception as e:
            logger.error(f"Failed to sync JSON to S3: {str(e)}")
