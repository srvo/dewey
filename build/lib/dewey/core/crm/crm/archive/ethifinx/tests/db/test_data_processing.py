...
from ethifinx.db.converters import workflow_to_database, database_to_workflow
...
def test_data_processor_workflow_data(sample_workflow_data):
...
def test_data_processor_research_data(sample_research_data):
...
def test_workflow_to_database_conversion(sample_workflow_data):
...
def test_workflow_to_database_with_history(sample_workflow_data):
    ...
    assert result["structured_data"]["history"][0]["previous_data"]["concern_level"] == 2


def test_database_to_workflow_conversion(sample_database_data):
...
def test_database_to_workflow_missing_data():
    """Test conversion with missing database fields."""
    invalid_data = {"structured_data": {}}
    with pytest.raises(KeyError):
        database_to_workflow(invalid_data)


def test_risk_score_calculation(sample_workflow_data):
...
def test_workflow_conversion_error_handling():
...
@pytest.mark.parametrize("source_text,expected_type", [
    ("api_data_source", "api_data"),
    ("research_report", "research_data"),
    ("unknown_source", "unknown")
])
def test_source_type_detection(source_text, expected_type):
