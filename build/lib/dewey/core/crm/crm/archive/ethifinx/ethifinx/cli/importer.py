...
def ensure_db_initialized():
    ...
    if not hasattr(ensure_db_initialized, "_initialized"):
        init_db()
        engine = get_connection().__enter__().get_bind()
        Base.metadata.create_all(engine)
        ensure_db_initialized._initialized = True


@cli.command()
@click.argument("file_path", type=click.Path(exists=True))
def import_universe(file_path: str):
    """Import universe data from a CSV file."""
    try:
        ensure_db_initialized()  # Initialize DB and create tables
...
@cli.command()
@click.argument("file_path", type=click.Path(exists=True))
def import_portfolio(file_path: str):
