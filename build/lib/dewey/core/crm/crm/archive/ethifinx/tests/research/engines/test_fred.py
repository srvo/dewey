...
@pytest.mark.asyncio
async def test_get_category(engine):
...
@pytest.mark.asyncio
async def test_get_category_children(engine):
...
@pytest.mark.asyncio
async def test_get_category_series(engine):
...
@pytest.mark.asyncio
async def test_get_series(engine):
...
@pytest.mark.asyncio
async def test_get_series_observations(engine):
...
@pytest.mark.asyncio
async def test_search_series(engine):
    """Test series search."""
    engine = await engine
    mock_response = {
        "seriess": [
...
@pytest.mark.asyncio
async def test_get_releases(engine):
...
@pytest.mark.asyncio
async def test_get_release_dates(engine):
...
async def test_search_retry_on_error(engine):
    ...
    with patch("aiohttp.ClientSession.get") as mock_get:
        ...
        mock_success_context.__aenter__.return_value.headers = {"Content-Type": "application/json"}
