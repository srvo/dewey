...
@pytest.mark.asyncio
async def test_web_search_basic(engine):
...
@pytest.mark.asyncio
async def test_web_search_with_options(engine):
...
@pytest.mark.asyncio
async def test_local_search_basic(engine):
...
@pytest.mark.asyncio
async def test_local_search_with_details(engine):
    ...
    with patch("aiohttp.ClientSession.get") as mock_get:
        def get_mock_response(url, **kwargs):
            ...
            mock_context = MagicMock()
            mock_context.__aenter__.return_value.status = 200
            mock_context.__aenter__.return_value.json.return_value = mock_responses[endpoint]
            return mock_context
