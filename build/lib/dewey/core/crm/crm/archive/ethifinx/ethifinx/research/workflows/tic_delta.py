...
class TICDeltaWorkflow:
    ...
    def _initialize_templates(self):
        ...
        self.engine.add_template("delta_analyzer", [
            {
                "role": "system",
    ...
    async def analyze_recent_changes(
        ...
        """
        with get_db_session() as session:
            # Get research results since last analysis
            recent_results = (
                session.query(ResearchResult)
                .filter(
                    ResearchResult.company_id == company_data["id"],
    ...
    async def run_delta_analysis(
        ...
        tic_history = await self.analyze_tic_history(
            company_id,
            lookback_days=lookback_days
        )

        # Analyze recent changes
        delta_analysis = await self.analyze_recent_changes(
            company_data,
            current_analysis.timestamp
        )
