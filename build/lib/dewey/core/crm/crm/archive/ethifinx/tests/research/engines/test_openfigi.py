...
@responses.activate
async def test_get_figi_data(engine, test_companies, mock_figi_response):
...
@responses.activate
async def test_get_figi_data_error(engine, test_companies):
...
@pytest.mark.parametrize("figi_data,expected_exchange", [
    ([
        {"exchCode": "US", "securityType": "Common Stock", "marketSector": "Equity"},
        {"exchCode": "LN", "securityType": "Common Stock", "marketSector": "Equity"}
    ], "US"),
    ([
        {"exchCode": "LN", "securityType": "Common Stock", "marketSector": "Equity"},
    ...
    ([
        {"exchCode": "LN", "securityType": "Common Stock", "marketSector": "Equity"},
        {"exchCode": "FR", "securityType": "Common Stock", "marketSector": "Equity"}
    ], "LN")
])
def test_filter_primary_listing(engine, figi_data, expected_exchange):
...
async def test_process_companies(engine, test_companies, mock_figi_response):
...
def test_respect_rate_limit(engine):
