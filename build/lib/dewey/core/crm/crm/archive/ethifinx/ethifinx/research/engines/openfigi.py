...
class OpenFIGIEngine(BaseEngine):
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError("OpenFIGI API key not provided and not found in environment")
    ...
    async def get_figi_data(self, companies: List[Dict[str, str]]) -> List[Dict[str, Any]]:
        ...
        while retries < self.max_retries:
            try:
                ...
                logger.error(f"Error processing batch (attempt {retries + 1}): {str(e)}")
                retries += 1
    ...
    async def process_companies(self, companies: List[Dict[str, str]], batch_size: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        for i in range(0, len(companies), batch_size):
            ...
            logger.info(f"Processing batch {i//batch_size + 1} of {(len(companies)-1)//batch_size + 1}")
