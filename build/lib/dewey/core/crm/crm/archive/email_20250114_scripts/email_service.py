...
class EmailService:
    ...
    def __init__(self, db_path: str = None, checkpoint_file: str = None, 
                 fetch_interval: float = 900, check_interval: float = 1.0):
        ...
        self.email_fetcher = EmailFetcher(
            db_path=config.DB_URL, checkpoint_file=config.CHECKPOINT_FILE
        )
    ...
    def fetch_cycle(self):
        ...
        try:
            ...
            logging.error(f"Error during fetch cycle: {str(e)}", exc_info=True)
    ...
    def run(self):
        ...
        try:
            ...
            logging.error(f"Fatal error in email service: {str(e)}", exc_info=True)
        finally:
            logging.info("Email service shutting down")


def run_service(interval_minutes: int = 15):
