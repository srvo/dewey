...
def get_or_create_contact(session, email: str, name: str = None) -> Contact:
    ...
    while True:
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if (
                        ...
                        continue
                    logger.error(f"HTTP error during fetch: {error}")
            ...
            if response is None:
                ...
                raise RateLimitError("API request failed after maximum retries")
...
def save_checkpoint(email_id: str):
    """Save the last successfully processed email ID."""
    with open("checkpoint.json", "w") as f:
        json.dump(
            {"last_email_id": email_id, "timestamp": datetime.now().isoformat()}, f
        )
...
def _process_emails_internal(service, session):
    ...
    try:
        ...
        for email_id_data in email_ids:
            try:
                ...
                logger.error(f"Failed to process email {email_id_data['id']}: {e}")
                continue
    except Exception as e:
