    ...
    def analyze_coverage(self) -> Dict[str, float]:
        ...
        try:
            ...
            for file_data in cov_data["files"].values():
                ...
                if filename.startswith("test_") or filename.startswith("_"):
                    continue
    ...
    def run(self):
        ...
        while context["current_index"] < len(context["plan"]):
            ...
            if current_item["status"] == "pending":
                try:
                    print(f"Generating tests for {current_item['script']}...")
                    success = self.generate_test_file(
                        current_item["script"], current_item["test_file"]
                    )

                    if success:
                        current_item["status"] = "completed"
                        context["stats"]["completed"] += 1
                        print(f"Successfully generated {current_item['test_file']}")
                    else:
                        current_item["status"] = "failed"
                        context["stats"]["failed"] += 1
                        print(f"Failed to generate tests for {current_item['script']}")
                    ...
                    print(
                        f"Error generating tests for {current_item['script']}: {str(e)}"
                    )
