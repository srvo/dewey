...
def call_aider(prompt: str, file_path: str) -> str:
    ...
    try:
        ...
        print(f"Error calling aider: {e.stderr}")
        return None
...
class PrecommitAnalyzer:
    ...
    def parse_ruff_issues(self, json_file: Path) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            self.print_status(f"Error parsing ruff output: {str(e)}", Status.FAILED)
            return {}
    ...
    def parse_flake8_output(self, json_file: Path) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            self.print_status(f"Error parsing flake8 output: {str(e)}", Status.FAILED)
            return {}
    ...
    def parse_format_output(self, json_file: Path) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            self.print_status(f"Error parsing format output: {str(e)}", Status.FAILED)
            return {}
    ...
    def print_status(self, message: str, status: Status = Status.PENDING):
        """Print colored status messages."""
        colors = {
            Status.PENDING: "\033[94m",  # Blue
            Status.IN_PROGRESS: "\033[33m",  # Yellow
            Status.COMPLETE: "\033[92m",  # Green
            Status.FAILED: "\033[91m",  # Red
        }
        reset = "\033[0m"
    ...
    def print_progress(self):
        ...
        print(f"\nProgress: |{bar}| {self.completed_issues}/{self.total_issues} issues")
        print(
            f"Success: {self.completed_issues - self.failed_issues} | Failed: {self.failed_issues}\n"
        )
    ...
    def run_ruff(self) -> Path:
        ...
        self.print_status(
            f"Running ruff checks (output to {output_file})...", Status.IN_PROGRESS
        )
        ...
        try:
            ...
            self.print_status(f"Ruff checks failed: {str(e)}", Status.FAILED)
            return None
    ...
    def _parse_failures(self, output: str) -> Dict[str, List[str]]:
        ...
        for line in output.splitlines():
            if line.startswith("Trim trailing whitespace"):
                current_file = None
            elif line.startswith("Check for merge conflicts"):
                current_file = None
    ...
    def create_fix_plan(self, tool_name: str, issues: Dict[str, List[Dict]]) -> FixPlan:
        ...
        for file_path, file_issues in issues.items():
            task = FixTask(
                file=file_path,
                issues=[f"{i.get('code', 'UNKNOWN')}: {i.get('message', 'No message')}" 
        ...
        self.print_status(f"Created fix plan with {len(fix_tasks)} tasks", Status.COMPLETE)
        return FixPlan(
            tool=tool_name,
            issues=fix_tasks,
    ...
    def save_fix_plan(self, plans: List[FixPlan]):
    ...
    def get_fix_template(self, issue_type: str) -> Optional[str]:
    ...
    def update_fix_template(self, issue_type: str, template: str):
        """Update or create a fix template."""
        template_file = self.template_dir / f"{issue_type}.py"
        with open(template_file, "w") as f:
            f.write(template)

    def generate_fix_template(self, issue_type: str) -> Optional[str]:
        ...
        try:
            ...
            output = call_aider(prompt, str(temp_file))
            ...
            self.print_status(f"Error generating template: {str(e)}", Status.FAILED)
            return None
        finally:
            if temp_file.exists():
                temp_file.unlink()

    def refresh_fix_templates(self):
        ...
        for issue_type in issue_types:
            self.print_status(
                f"Generating template for {issue_type}...", Status.IN_PROGRESS
            )
            ...
            if template:
                ...
                self.print_status(
                    f"Failed to generate template for {issue_type}", Status.FAILED
                )
    ...
    def apply_fix(self, file_path: str, issue: str, template: str) -> bool:
        ...
        try:
            ...
            self.print_status(f"Error applying fix: {str(e)}", Status.FAILED)
            return False
        finally:
    ...
    def process_fix_task(self, task: FixTask) -> bool:
        ...
        self.print_status(
            f"Processing {task.file} ({len(task.issues)} issues)", Status.IN_PROGRESS
        )
        ...
        try:
            for issue in task.issues:
                ...
                if not template:
                    self.print_status(
                        f"No template found for {issue_type}", Status.FAILED
                    )
            ...
            self.print_status(
                f"Completed {task.file} in {duration:.2f}s", Status.COMPLETE
            )
            return True
            ...
            task.end_time = datetime.now()
            self.print_status(f"Failed to process {task.file}: {str(e)}", Status.FAILED)
            return False
        ...
        for tool in self.tools:
            ...
            if output_file.exists():
                try:
                    ...
                    self.print_status(f"Error removing {tool} output: {str(e)}", Status.FAILED)
    ...
    def process_tasks(self, plans: List[FixPlan]):
        ...
        for plan in plans:
            ...
            if any(issue.fixable for issue in plan.issues):
                self.print_status(f"Running fix command: {plan.fix_command}", Status.IN_PROGRESS)
                try:
                    subprocess.run(plan.fix_command.split(), check=True)
                    self.print_status(f"Fix command completed for {plan.tool}", Status.COMPLETE)
                except subprocess.CalledProcessError as e:
                    self.print_status(f"Fix command failed for {plan.tool}: {str(e)}", Status.FAILED)
    ...
    def run(self):
        ...
        try:
            ...
            self.print_status(f"\nError during analysis: {str(e)}", Status.FAILED)
            sys.exit(1)
        finally:
            self.cleanup()


    def run_tool(self, tool_name: str) -> Optional[Path]:
        ...
        try:
            ...
            if tool_name == "flake8":
                ...
                try:
                    ...
                    self.print_status(f"Flake8 JSON decode error: {str(e)}", Status.FAILED)
                    return None
            ...
            self.print_status(f"{tool_name} failed: {str(e)}", Status.FAILED)
            return None
