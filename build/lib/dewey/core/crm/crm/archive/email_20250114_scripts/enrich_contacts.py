...
def create_enrichment_task(
    ...
    try:
        ...
        logger.error(f"[TASK] Failed to create task: {str(e)}", exc_info=True)
        raise
...
def update_task_status(
    ...
    try:
        ...
        logger.error(f"[TASK] Failed to update task status: {str(e)}", exc_info=True)
        raise
...
def store_enrichment_source(
    ...
    try:
        ...
        logger.error(f"[SOURCE] Failed to store source: {str(e)}", exc_info=True)
        raise


def extract_contact_info(message_text: str) -> Optional[Dict]:
    ...
    try:
        ...
        logger.error(
            f"[EXTRACT] Error extracting contact info: {str(e)}", exc_info=True
        )
        return None
...
def process_email_for_enrichment(conn, email_id: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"[PROCESS] Error processing email {email_id}: {str(e)}", exc_info=True
            )
        ...
        logger.error(
            f"[PROCESS] Fatal error processing email {email_id}: {str(e)}",
            exc_info=True,
        )
        return False


def enrich_contacts(batch_size: int = None):
    ...
    try:
        ...
        logger.error(
            f"[BATCH] Fatal error in enrichment batch: {str(e)}", exc_info=True
        )
        raise
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            f"[MAIN] Contact enrichment process failed: {str(e)}", exc_info=True
        )
