...
class EmailAnalyzer:
    ...
    def __init__(self, default_model: ModelType = ModelType.NEMO):
        ...
        if not self.api_key:
            raise ValueError("DEEPINFRA_API_KEY not set in .env file")
        if len(self.api_key) < 32:
            raise ValueError("DEEPINFRA_API_KEY appears invalid - check .env file")
    ...
    def analyze_email(
        ...
        if not self.api_key or len(self.api_key) < 32:
            raise ValueError("Invalid DEEPINFRA_API_KEY format")
            ...
            if sender_history["is_known_sender"]:
                ...
                if "human_interaction" in analysis["scores"]:
                    analysis["scores"]["human_interaction"]["score"] = min(
                        1.0, analysis["scores"]["human_interaction"]["score"] * 1.2
                    )
                ...
                if "automation_score" in analysis["scores"]:
                    analysis["scores"]["automation_score"]["score"] = max(
                        0.0, analysis["scores"]["automation_score"]["score"] * 0.8
                    )
