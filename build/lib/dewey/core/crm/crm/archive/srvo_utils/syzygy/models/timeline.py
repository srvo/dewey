...
import uuid
from .base import TimelineEvent
...
class TimelineView(models.Model):
    ...
    importance_level = models.IntegerField(
        default=0, help_text="0=normal, 1=important, 2=critical"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["event_type", "created_at"], name="timeline_type_date_idx"
            ),
            models.Index(
                fields=["user_id", "created_at"], name="timeline_user_date_idx"
            ),
        ]
...
class Transcript(models.Model):
    ...
    security = models.ForeignKey(
        Universe, null=True, blank=True, on_delete=models.SET_NULL
    )
...
class TranscriptAnalysis(models.Model):
    ...
    transcript = models.OneToOneField(
        Transcript, null=True, blank=True, on_delete=models.CASCADE
    )
