...
from email_processing.gmail_client import GmailClient, SCOPES
import base64
import json
import structlog
import pickle
...
def test_authentication_success(gmail_client, mock_service_account_creds, mock_service):
...
def test_authentication_failure(gmail_client):
...
def test_fetch_emails_with_date_filter(gmail_client, mock_service):
...
def test_save_checkpoint(gmail_client):
...
def test_load_checkpoint(gmail_client):
...
def test_error_handling_fetch_emails(gmail_client, mock_service):
...
def test_checkpoint_file_error_handling(gmail_client):
