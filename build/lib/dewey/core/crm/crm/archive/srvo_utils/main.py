...
def verify_database_schema() -> bool:
    ...
    with connection.cursor() as cursor:
        ...
        if missing_tables:
            raise CommandError(f"Missing required tables: {', '.join(missing_tables)}")
        ...
        if not result or result[0] == 0:
            raise CommandError("Missing unique constraint on emails.gmail_id")
        ...
        for table in sorted(required_tables):  # Sort tables for consistent order
            ...
            if missing_columns:
                raise CommandError(
                    f"Table {table} missing columns: {', '.join(sorted(missing_columns))}"
                )

    return True


def sync_gmail_history(last_history_id: str = None) -> bool:
    ...
    try:
        ...
        logging.error(f"Error syncing Gmail history: {str(e)}")
        return False
...
def verify_gmail_id_tracking() -> bool:
    ...
    try:
        ...
        logging.error(f"Error verifying Gmail ID tracking: {str(e)}")
        return False


def process_message_added(message_data):
...
def process_message_removed(message_data):
    ...
    if email:
        ...
        association = MessageThreadAssociation.objects.filter(
            message_id=email.email_id, valid_to=None
        ).first()
...
def process_label_changes(change_data):
...
def update_last_history_id(history_id):
    """Update the last processed history ID"""
    config, created = Config.objects.get_or_create(
        key="last_history_id", defaults={"value": history_id}
    )
    if not created:
