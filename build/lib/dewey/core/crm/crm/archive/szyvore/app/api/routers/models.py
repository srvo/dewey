...
class SourceNodes(BaseModel):
    ...
    def from_source_node(cls, source_node: NodeWithScore):
        ...
        financial_metadata = {
            ...
            "financial_year": int(metadata.get("financial_year"))
            if metadata.get("financial_year")
            else None,
            "currency": metadata.get("currency"),
            "total_assets": float(metadata.get("total_assets"))
            if metadata.get("total_assets")
            else None,
            "total_liabilities": float(metadata.get("total_liabilities"))
            if metadata.get("total_liabilities")
            else None,
            "net_worth": float(metadata.get("net_worth"))
            if metadata.get("net_worth")
            else None,
            "financial_ratios": {
                "debt_to_equity": None,
                "current_ratio": None,
