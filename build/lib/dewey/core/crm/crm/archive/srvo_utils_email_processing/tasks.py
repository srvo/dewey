...
import sentry_sdk
from .gmail_history_sync import sync_gmail_history, full_sync_gmail, get_gmail_service
from database.models import Email, Contact, EventLog, EnrichmentTask
...
def sync_gmail_history_task(self):
    """
...
def enrich_contacts_task(self, batch_size: int = 50):
    """
    ...
    try:
        ...
        logger.error(
            "contact_enrichment_task_failed", error=str(e), error_type=type(e).__name__
        )
        # Create error event
        EventLog.objects.create(
...
def backfill_email_metadata_task(self):
    ...
    try:
        ...
        ).order_by("-received_at")[:100]  # Process in batches of 100
        ...
        logger.error("backfill_failed", error=str(e), error_type=type(e).__name__)
        EventLog.objects.create(
            event_type="BACKFILL_ERROR",
            details={
                "error": str(e),
