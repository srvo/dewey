...
from database.models import Email, EventLog, RawEmail, EmailLabelHistory
...
def _execute_gmail_api(request):
    ...
    try:
        lock_acquired = redis_client.set(
            f"{WORKER_LOCK_KEY}:{lock_key}", "1", ex=WINDOW_DURATION, nx=True
        )
    ...
    pipe.zrangebyscore(
        f"{RATE_LIMIT_KEY}:{lock_key}", now - WINDOW_DURATION, float("inf")
    )
    ...
    try:
        ...
        redis_client.zremrangebyscore(
            f"{RATE_LIMIT_KEY}:{lock_key}", 0, now - WINDOW_DURATION
        )
        return result
        ...
        if "quota" in str(e).lower() or "rate limit exceeded" in str(e).lower():
            ...
            pipe.set(
                f"{BACKOFF_KEY}:{lock_key}", max(float(current_backoff), ERROR_COOLDOWN)
            )
            pipe.execute()
...
def full_sync_gmail():
    ...
    try:
                    ...
                    try:
                        ...
                        with transaction.atomic():
                            ...
                            received_at = timezone.datetime.fromtimestamp(
                                int(message_data["internalDate"]) / 1000, tz=pytz.UTC
                            )
        ...
        logger.error("full_sync_failed", error_type=type(e).__name__, error=str(e))
        EventLog.objects.create(
            event_type="SYNC_ERROR",
            details={
                "error_message": str(e),
...
def process_history_item(service, history):
    ...
    try:
        ...
        for source in ["messages", "messagesAdded", "messagesDeleted"]:
            if source in history and history[source]:
            ...
            if email:
                ...
                if message_data:
                    ...
                    email.size_estimate = message_data.get(
                        "sizeEstimate", email.size_estimate
                    )
