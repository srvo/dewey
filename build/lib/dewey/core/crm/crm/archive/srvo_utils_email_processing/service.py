...
from database.models import Email, Contact, EmailContactAssociation
...
class EmailProcessingService:
    ...
    def _parse_received_date(self, headers: Dict) -> datetime:
        ...
        if "date" in headers:
            try:
                ...
                logger.warning(
                    "Failed to parse email date", date_str=headers["date"], error=str(e)
                )
        return received_date

    def _create_email_record(
        self, email_data: Dict, headers: Dict, received_date: datetime
    ) -> Email:
    ...
    def _process_contacts(self, email: Email, headers: Dict):
        ...
        if from_email:
            ...
            EmailContactAssociation.objects.create(
                email=email, contact=from_contact, association_type="from"
            )
            ...
            if field in headers:
                ...
                for recipient_email in emails:
                    if recipient_email:
                        ...
                        EmailContactAssociation.objects.create(
                            email=email, contact=contact, association_type=field
                        )

    def _extract_email_address(self, address_str: str) -> Optional[str]:
