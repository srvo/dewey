...
class ContactMerger:
    ...
    async def find_merge_candidates(self, contact_id: str) -> List[Dict]:
        ...
        try:
            ...
            target_enrichment = EnrichmentSource.objects.filter(
                entity_type="contact", entity_id=contact_id, valid_to__isnull=True
            ).first()

            # Find potential candidates
            candidates = []

            # First, check email pattern matches
            ...
            for candidate in similar_emails:
                ...
                contact2_data = {
                    ...
                    "name": candidate.name,
                    "enrichment": candidate_enrichment.data
                    if candidate_enrichment
                    else {},
                }

                # Analyze with AI
                similarity = await self.agent.analyze_contacts(
                    contact1_data, contact2_data
                )

                if similarity.should_merge or similarity.confidence < 0.9:
                    candidates.append(
                        {
    ...
    async def merge_contacts(self, primary_id: str, secondary_id: str):
        ...
        try:
            with transaction.atomic():
                ...
                # Archive secondary contact's enrichment sources
                EnrichmentSource.objects.filter(
                    entity_type="contact", entity_id=secondary_id, valid_to__isnull=True
                ).update(valid_to=timezone.now())

                # Create new enrichment source for merged data
