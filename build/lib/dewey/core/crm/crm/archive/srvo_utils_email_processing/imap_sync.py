            ...
            for msg_id in messages:
                try:
                    ...
                    msg_data = self.client.fetch(
                        [msg_id], ["RFC822", "FLAGS", "INTERNALDATE"]
                    )
                    if not msg_data:
                        continue
                    ...
                    try:
                        ...
                        logger.info(
                            "Updating existing email: %s", email_message["Message-ID"]
                        )
                    except Email.DoesNotExist:
                        logger.info(
                            "Creating new email: %s", email_message["Message-ID"]
                        )
                    ...
                    RawEmail.objects.update_or_create(
                        email=email_obj, defaults={"raw_data": raw_email}
                    )

                except Exception as e:
                    logger.error("Failed to process email %s: %s", msg_id, str(e))
                    continue

        except Exception as e:
            logger.error("Failed to sync folder %s: %s", folder, str(e))
            raise
        finally:
